...
`/v3/authenticate` 与 `/v3/acl` 服务。推荐生产入口是 `flowie_server --control-config`，由 Server
...
控制进程只监听一个显式 HTTPS 地址。首位管理员通过一次性 YAML bootstrap 和 DotEnv 密码建立；
...
根节点只接受 `version`、`listener`、`storage`、`bootstrap`、`management`、`dashboard` 和 `auth`；
...
| `bootstrap.username` | 系统管理员用户名，默认 `admin`；固定属于内部 `system` 管理域 |
| `bootstrap.password_ref` | 默认 `env://FLOWIE_BOOTSTRAP_PASSWORD`；密码长度 `16..4096`，YAML literal 被拒绝 |
| `management.rpc_path` | 静态绝对 path，默认 `/v1/management/rpc` |
| `management.session.capacity` | 有界登录会话数，默认 `1024`，最大 `65536` |
| `management.session.max_sessions_per_principal` | 每个 `(root_group, principal)` 最多保留的会话数，默认 `5`，最大 `65536`；新登录撤销该主体最早签发的会话 |
...
| `auth.service_bindings` | `service_id`、`token_ref`、`root_group` 的唯一绑定，最多 32 个 |
...
bootstrap 只解决空 Repository 尚无系统管理员可登录的问题，不是第二条管理通道。系统管理员固定属于
内部 `system` 管理域，不属于 `root-a/root-b/...` 任一业务域。DotEnv 只提供一次性密码；runtime 仍通过
同一 Repository command/audit 边界创建 `system` Root Group、启用用户、本地 credential、
`system_admin` 与 `password_change_required` Role 及 assignment，不直接执行数据库 SQL：
...
```dotenv
FLOWIE_BOOTSTRAP_PASSWORD=replace-with-a-random-password-of-at-least-16-bytes
```

启动顺序固定为：创建/验证 Repository schema，执行 bootstrap，创建管理 session store，最后打开 listener。
七个领域命令具有固定 request ID 和 revision，因此进程在中途退出后可安全重放。初始密码只参与首次
credential 创建；bootstrap 完成后重启只验证系统管理员结构，不用初始密码覆盖或验证当前密码。
bootstrap 成功后，runtime 会清除当前进程环境中的密码，但不会改写 DotEnv 文件。

首次登录 `system/admin` 后只能进入 `/v1/management/password`；改密成功会轮换 credential、移除
`password_change_required` 并撤销当前 session。重新登录后，系统管理员通过 `flowie.root.create` 创建
`root-a/root-b/...`，再显式为目标 root 创建首位用户，通过 `flowie.password.set` 的 `create` mode
设置人类密码，并创建、分配 `security_admin` 角色。管理员重置已有密码时必须显式使用 `replace`
mode，不会从 create 失败自动降级为替换。之后应停止进程，删除 YAML 的 `bootstrap`
block 和 DotEnv 中的 `FLOWIE_BOOTSTRAP_PASSWORD`，把 PostgreSQL `schema_mode` 从迁移任务使用的
`migrate` 恢复为 `validate`，再启动正式实例。之后只通过登录 session 保护的 JSON-RPC 创建/禁用用户，生成/轮换/
撤销 credential，维护 Group/Role，验证并发布 ACL；不得再次添加 bootstrap，也不得直接修改数据库。

仓库提供一次性 operator 工具执行上述五个 RPC。密码只从当前进程环境读取，不接受命令行密码参数；
每个命令使用由 system principal、Root Group 和首位管理员派生的固定 request ID。重复执行会走
Repository replay；每步之后重新读取全局 revision，因此中断后可继续，最终还会以新业务管理员重新登录
验证 credential 与 Root Group 绑定：
...
`system_admin` 凭据，也不得自行创建 Root Group；它们只使用已 provision 的最小权限业务管理员或
service credential。
...
外层 `auth.service_bindings[].token_ref` 保护 Broker 到 `/v3/authenticate` 和 `/v3/acl` 的请求；内层
`auth.external_https.service_token_ref` 保护 `flowie-control` 到第三方断言服务的请求。两个 token
具有不同的信任方向、权限和轮换周期，不能复用。

`GET /v3/acl` 只接受受信 service bearer。Root Group 由命中的 service binding 解析，客户端不能通过
...
`/v1/management/login` 提交 Root Group、principal 和密码；认证成功后服务端签发随机不透明 token。
...
```text
verified login credential in the presented Root Group
...
`password_change_required` 存在时会屏蔽其他管理权限，直到 `flowie.password.change` 或 Dashboard
改密页成功完成 credential rotate 与角色移除。`system_admin` 只能位于 `system` 域，可创建业务 Root
Group 并显式管理目标域；业务 `security_admin` 永远不能跨 Root Group。任一步失败都返回未授权。
...
Management RPC 当前注册 32 个方法，其中 `flowie.root_group.list` 只允许登录在 `system` Root 且拥有
`system_admin` 的 caller 调用。它以 Repository 中已启用、无 parent、depth 为 0 且
`group_id == root_group_id` 的根 `security_group` 行作为 Root Group 唯一事实源，并提供 `after`/`limit`
keyset 分页。

支持目标作用域的 read/list/write RPC 可携带可选 `root_group_id`；省略时使用登录 session 的 Root。
...
普通 `security_admin` 即使伪造其他 `root_group_id` 也会得到 JSON-RPC forbidden `-32003`。目标 Root
...
build\Msvc-Release\bin\flowie_server.exe `
  ...
  --require-security `
  C:\flowie\flowie.yml C:\flowie\flowie.flow
```
...
Dashboard 入口为 `/v1/management/dashboard`，只显示状态概览；管理数据集使用独立页面：
`/v1/management/dashboard/users`、`/groups`、`/roles`、`/acls` 和 `/audit`（均以 Dashboard 入口为
路径前缀）。页面共享登录 session、CSRF 保护和领域 command/query service，但每次只读取并渲染当前
数据集。`system/system_admin` 会看到 Root Group 选择器：它提示 `flowie.root_group.list` 返回的前
100 个 Root，也接受手工输入其他已存在 Root；当前 `root_group_id` 会保留在页面导航、查询、keyset
...
Members 写入口。
RPC 使用配置的 `management.rpc_path`，认证服务固定为 `POST /v3/authenticate`。反向代理必须
