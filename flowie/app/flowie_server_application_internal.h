...
#define FLOWIE_SERVER_APPLICATION_CONFIG_INIT                                                     \
  {sizeof(flowie_server_application_config_t), "flowie", NULL, NULL, NULL, 0u, 0, NULL}
...
#define FLOWIE_SERVER_APPLICATION_ERROR_INIT                                                      \
  {                                                                                                \
    sizeof(flowie_server_application_error_t), NULL, TURBO_OK,                                    \
        FLOWIE_SERVER_APPLICATION_ERROR_NONE, FLOWIE_WORKER_ERROR_INIT, {0}, {0}                   \
  }

/**
 * Compose storage, product adapters, HTTPS Auth/ACL factories, one prepared worker, and an
 * optional embedded Control runtime configuration.
 */
...
/** Start and stop the embedded Control listener and prepared MQTT worker. Calls are serialized. */
