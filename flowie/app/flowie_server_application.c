...
static void flowie_server_application_error_set_worker(
    flowie_server_application_error_t *error, const flowie_worker_error_t *worker) {
...
static void flowie_server_application_error_set_control_config(
    ...
    const flowie_control_config_error_t *control_error) {
...
static void flowie_server_application_error_set_control(
    flowie_server_application_error_t *error, const char *operation, int status) {
...
static int flowie_server_register_mqtt_client_adapter(
    void *ctx, turbo_flow_t *flow, const turbo_flow_resolved_config_t *resolved,
    const char *adapter_name, turbo_flow_config_error_t *error) {
  (void)ctx;
...
int flowie_server_application_create(const flowie_server_application_config_t *config,
  ...
  if (!config || config->size != sizeof(*config) || !out || !config->profile ||
      ...
      (config->storage_backend_plugin_path_count > 0u &&
       !config->storage_backend_plugin_paths)) {
  ...
  for (size_t i = 0u; i < config->storage_backend_plugin_path_count; ++i) {
    if (!config->storage_backend_plugin_paths[i] ||
        !config->storage_backend_plugin_paths[i][0]) {
  ...
  for (size_t i = 0u; i < config->storage_backend_plugin_path_count; ++i) {
    ...
    rc = turbo_flow_storage_backend_registry_load(
        application->storage_backends, config->storage_backend_plugin_paths[i], plugin_error,
        sizeof(plugin_error));
    if (rc != TURBO_OK) {
