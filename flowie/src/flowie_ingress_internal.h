...
#define FLOWIE_INGRESS_INTERNAL_ABI_V1 1u
#define FLOWIE_INGRESS_INTERNAL_ABI_V2 2u
...
/** Same-lane completion hook called once after a packet selected for graph publication. */
...
#define FLOWIE_INGRESS_CONFIG_INIT                                                                 \
  {sizeof(flowie_ingress_config_t), FLOWIE_INGRESS_INTERNAL_ABI_V2, NULL, NULL,                    \
 ...
 * Complete packets are published to the configured source.
 * `published` counts only successful graph publications from this call.
 * A protocol, HWM, or graph error is terminal; destroy the ingress before accepting more bytes.
 */
