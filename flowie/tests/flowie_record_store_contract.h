#ifndef FLOWIE_RECORD_STORE_CONTRACT_H
#define FLOWIE_RECORD_STORE_CONTRACT_H
...
typedef struct flowie_record_store_contract_result_s {
  ...
  size_t mqtt_wire_size;
  int mqtt_wire_equal;
} flowie_record_store_contract_result_t;

typedef struct flowie_record_store_contract_capture_s {
  size_t count;
  int valid;
} flowie_record_store_contract_capture_t;

typedef struct flowie_record_store_recovery_result_s {
  int timeout_status;
...
} flowie_record_store_recovery_result_t;

typedef struct flowie_record_store_fault_s {
  turbo_flow_record_store_t *inner;
  int fail_before_commit;
  int lose_commit_reply;
} flowie_record_store_fault_t;

typedef struct flowie_record_store_revision_capture_s {
  const uint8_t *expected_key;
  size_t expected_key_size;
  uint64_t revision;
  size_t count;
} flowie_record_store_revision_capture_t;

static inline int flowie_record_store_fault_scan(void *ctx, turbo_flow_record_visit_fn visit,
                                                 void *visit_ctx) {
  flowie_record_store_fault_t *fault = (flowie_record_store_fault_t *)ctx;
  return fault && fault->inner && fault->inner->scan
             ? fault->inner->scan(fault->inner->ctx, visit, visit_ctx)
             : TURBO_EINVAL;
}

static inline int flowie_record_store_fault_commit(void *ctx,
...
static inline int flowie_record_store_revision_visit(void *ctx,
                                                     const turbo_flow_record_view_t *record) {
  flowie_record_store_revision_capture_t *capture =
      (flowie_record_store_revision_capture_t *)ctx;
...
static inline int flowie_record_store_recovery_revision(turbo_flow_record_store_t *store,
                                                        const uint8_t *key, size_t key_size,
                                                        uint64_t *revision_out) {
  flowie_record_store_revision_capture_t capture = {key, key_size, 0u, 0u};
  int rc;
  if (!store || !revision_out) return TURBO_EINVAL;
  rc = store->scan(store->ctx, flowie_record_store_revision_visit, &capture);
...
static inline int flowie_record_store_recovery_contract_run(
    turbo_flow_record_store_t *store, flowie_record_store_recovery_result_t *result) {
  ...
  flowie_record_store_fault_t fault = {store, 0, 0};
  ...
  fault_store.scan = flowie_record_store_fault_scan;
  fault_store.commit = flowie_record_store_fault_commit;
  ...
  rc = flowie_record_store_recovery_revision(store, key, sizeof(key),
...
static inline int flowie_record_store_contract_visit(void *ctx,
                                                     const turbo_flow_record_view_t *record) {
  ...
  flowie_record_store_contract_capture_t *capture =
      (flowie_record_store_contract_capture_t *)ctx;
...
static inline int flowie_record_store_contract_count(void *ctx,
                                                     const turbo_flow_record_view_t *record) {
...
static inline int flowie_record_store_contract_run(
    turbo_flow_record_store_t *store, flowie_record_store_contract_result_t *result) {
  ...
  static const uint8_t second_value[] = {0x32u, 0x0au, 0x00u, 0x01u, 't', 0x00u,
                                         0x01u, 0x00u, 'v',   0x00u, '2', 0xfeu};
  turbo_flow_record_mutation_t mutation = TURBO_FLOW_RECORD_MUTATION_INIT;
  flowie_record_store_contract_capture_t capture = {0};
  size_t count = 0u;
  int rc;
  if (!store || !result || !store->scan || !store->commit ||
    ...
    return TURBO_EINVAL;
  memset(result, 0, sizeof(*result));
  rc = store->scan(store->ctx, flowie_record_store_contract_count, &count);
  ...
  rc = store->scan(store->ctx, flowie_record_store_contract_visit, &capture);
  if (rc != TURBO_OK || capture.count != 1u || !capture.valid)
    return rc == TURBO_OK ? TURBO_EPROTO : rc;
  result->restored_count = capture.count;
  result->restored_revision = 2u;
  result->mqtt_wire_size = sizeof(second_value);
  result->mqtt_wire_equal = capture.valid;
