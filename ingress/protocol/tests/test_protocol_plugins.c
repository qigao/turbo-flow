...
spec("protocol plugin conformance") {
  it("loads six equal DLLs and raw-preserves each protocol frame") {
    ...
    const size_t gbt32960_size =
        protocol_gbt32960_frame(gbt32960, sizeof(gbt32960));
    const size_t jtt808_size =
        protocol_jtt808_frame(jtt808, sizeof(jtt808));
    protocol_case_t cases[] = {
        {FLOW_PROTOCOL_MQTT_SN_MODULE, "mqtt-sn",
         TURBO_FLOW_PROTOCOL_MQTT_SN, "1.2", "sensor-1",
        ...
        {FLOW_PROTOCOL_LWM2M_MODULE, "lwm2m",
         TURBO_FLOW_PROTOCOL_LWM2M, "1.2.2", "device-3", "write",
         lwm2m, sizeof(lwm2m)},
        {FLOW_PROTOCOL_OCPP_MODULE, "ocpp", TURBO_FLOW_PROTOCOL_OCPP,
         "1.6J", "charger-4", "BootNotification", ocpp,
         sizeof(ocpp) - 1u},
        {FLOW_PROTOCOL_GBT32960_MODULE, "gbt32960",
         TURBO_FLOW_PROTOCOL_GBT_32960, "2025", NULL,
         "realtime-data", gbt32960, gbt32960_size},
        {FLOW_PROTOCOL_JTT808_MODULE, "jtt808",
         TURBO_FLOW_PROTOCOL_JTT_808, "2019-A1", NULL, "location",
         jtt808, jtt808_size}};
    turbo_flow_protocol_registry_t *registry = NULL;
    char reason[256];
  ...
  it("emits protocol responses only after an explicit settlement result") {
    ...
    owner = NULL;
    protocol = NULL;
    output = (turbo_flow_protocol_frame_output_t)
        TURBO_FLOW_PROTOCOL_FRAME_OUTPUT_INIT;
    output.data = response;
    output.capacity = sizeof(response);
    check_int_eq(protocol_open_one(
  ...
  it("encodes bounded semantic downlink commands for every plugin") {
    ...
    struct command_case_s {
      ...
      size_t payload_size;
        ...
        {FLOW_PROTOCOL_MQTT_SN_MODULE, "mqtt-sn",
         TURBO_FLOW_PROTOCOL_MQTT_SN, "1.2", "sensor-1", "publish",
         "7", NULL, 9u, value, sizeof(value)},
        {FLOW_PROTOCOL_COAP_MODULE, "coap",
         TURBO_FLOW_PROTOCOL_COAP, "RFC7252", "sensor-2", "get",
         "/sensors/1", "tk", 0x1234u, NULL, 0u},
        {FLOW_PROTOCOL_LWM2M_MODULE, "lwm2m",
         TURBO_FLOW_PROTOCOL_LWM2M, "1.2.2", "device-3", "read",
         "/3/0/13", "lm", 0x1235u, NULL, 0u},
        {FLOW_PROTOCOL_OCPP_MODULE, "ocpp",
         TURBO_FLOW_PROTOCOL_OCPP, "2.0.1", "charger-4",
    ...
    for (size_t i = 0u; i < sizeof(cases) / sizeof(cases[0]); ++i) {
      ...
      turbo_flow_protocol_owner_t *owner = NULL;
      turbo_flow_protocol_t *protocol = NULL;
      turbo_flow_protocol_command_view_t command =
          TURBO_FLOW_PROTOCOL_COMMAND_VIEW_INIT;
      turbo_flow_protocol_frame_output_t output =
          TURBO_FLOW_PROTOCOL_FRAME_OUTPUT_INIT;
      uint8_t frame[512];
      check_int_eq(protocol_open_one(
