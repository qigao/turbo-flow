...
int flow_protocol_metadata_text_n(char *out, size_t capacity, const char *text,
                                 size_t length) {
  if (!out || capacity == 0u || !text || length == 0u) return SALTS_EINVAL;
  if (length >= capacity) return SALTS_EMSGSIZE;
  if (memchr(text, '\0', length) != NULL) return SALTS_EPROTO;
...
int flow_protocol_metadata_format(char *out, size_t capacity,
  ...
  const int written =
      out && capacity > 0u && format
          ...
          : -1;
  return written > 0 && (size_t)written < capacity ? SALTS_OK
                                                   : SALTS_EMSGSIZE;
}
...
int flow_protocol_coap_inspect(const turbo_flow_protocol_frame_view_t *frame,
  ...
  while (offset < frame->data_size) {
    ...
    if (delta == 13u) {
      ...
      if (frame->data_size - offset < 2u) return SALTS_EPROTO;
...
static uint8_t flow_protocol_coap_response_code(uint8_t request_code,
                                               int status) {
  if (status != SALTS_OK) return UINT8_C(0xa0); /* 5.00 */
  switch (request_code) {
  case 0u:
    return 0u;
  case 1u:
...
int flow_protocol_coap_reply(
  ...
  if (response_size > output->capacity) return SALTS_EMSGSIZE;
  ...
  if (type == 0u) {
    ...
    if (separate_message_id == 0u) return SALTS_ERANGE;
...
static int flow_protocol_coap_option_write(uint8_t *data, size_t capacity,
  ...
  if (!data || !offset || (!value && value_size != 0u)) return SALTS_EINVAL;
  if (delta < 13u)
  ...
  if (required > capacity - *offset) return SALTS_EMSGSIZE;
...
int flow_protocol_coap_encode(
  ...
  if (token_size > 8u) return SALTS_EMSGSIZE;
  message_id = (uint16_t)command->sequence;
  if (message_id == 0u) return SALTS_EINVAL;
  if (4u + token_size > output->capacity) return SALTS_EMSGSIZE;
  ...
  if (*resource == '\0') return SALTS_EINVAL;
  segment = resource;
  for (;;) {
    const char *slash = strchr(segment, '/');
    const size_t segment_size =
        slash ? (size_t)(slash - segment) : strlen(segment);
    if (segment_size == 0u) return SALTS_EINVAL;
