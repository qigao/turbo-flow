...
static int flow_protocol_runtime_text_copy(char *out, size_t capacity, const char *text,
                                           int optional) {
  size_t length;
  if (!out || capacity == 0u || (!optional && (!text || !text[0]))) return TURBO_EINVAL;
  if (!text || !text[0]) {
    out[0] = '\0';
    return TURBO_OK;
  }
...
static int flow_protocol_runtime_frame_size(const turbo_flow_protocol_runtime_t *runtime,
  ...
  *out = 0u;
  ...
  case TURBO_FLOW_PROTOCOL_GBT_32960: {
    ...
    if (data[0] != 0x23u || (size >= 2u && data[1] != 0x23u)) return TURBO_EPROTO;
    ...
    if (frame_size > runtime->config.max_frame_size) return TURBO_EMSGSIZE;
  ...
  case TURBO_FLOW_PROTOCOL_JTT_808:
    ...
    return size == runtime->config.max_frame_size ? TURBO_EMSGSIZE : TURBO_OK;
  default:
  ...
  *admitted = 1;
  if (disposition == TURBO_FLOW_PROTOCOL_PUBLISH_PENDING) {
    ...
    if (runtime->ops.reply &&
      ...
      rc = turbo_flow_protocol_reply(runtime->protocol, &reply_request, TURBO_OK, &reply);
      if (rc != TURBO_OK) return rc;
    ...
    if (runtime->ops.settled)
      runtime->ops.settled(runtime->callback_ctx, session->session_id, session->generation,
                           delivery_id, &message.metadata, TURBO_OK);
  }
  return TURBO_OK;
}
...
static int flow_protocol_runtime_pump(turbo_flow_protocol_runtime_t *runtime,
  ...
  while (session->state == TURBO_FLOW_PROTOCOL_SESSION_OPEN && session->buffered > 0u) {
    ...
    if (rc == TURBO_EBUSY || rc == TURBO_ENOSPC || rc == TURBO_ENOBUFS) {
...
int turbo_flow_protocol_runtime_create(turbo_flow_protocol_t *protocol,
  ...
  if (rc != TURBO_OK) return rc;
  if (config->max_frame_size > info.max_frame_size) return TURBO_ERANGE;
  if (config->max_sessions == SIZE_MAX) return TURBO_EMSGSIZE;
  buffer_count = config->max_sessions + 1u;
  if (config->max_frame_size > SIZE_MAX / buffer_count) return TURBO_EMSGSIZE;
  buffer_bytes = buffer_count * config->max_frame_size;
  if (buffer_bytes > config->max_buffered_bytes) return TURBO_ENOSPC;
...
int turbo_flow_protocol_runtime_session_open(
  ...
  if (session) return session->generation == request->generation ? TURBO_EALREADY : TURBO_EBUSY;
  ...
  if (strcmp(version, runtime->protocol_info.protocol_version) != 0) return TURBO_ENOTSUP;
...
int turbo_flow_protocol_runtime_session_feed(turbo_flow_protocol_runtime_t *runtime,
  ...
  if (rc != TURBO_OK || !runtime || session_id == 0u || generation == 0u || (!data && size != 0u))
    return TURBO_EINVAL;
  if (!runtime->accepting) return TURBO_ESHUTDOWN;
  ...
  if (session->state == TURBO_FLOW_PROTOCOL_SESSION_DRAINING) return TURBO_ESHUTDOWN;
  ...
  if (session->buffer_offset > runtime->config.max_frame_size ||
    ...
    return TURBO_EPROTO;
  if (size > runtime->config.max_frame_size - session->buffered) return TURBO_ENOBUFS;
  if (size > 0u) {
...
int turbo_flow_protocol_runtime_settle(turbo_flow_protocol_runtime_t *runtime, uint64_t delivery_id,
  ...
  int rc = flow_protocol_runtime_result_validate(result);
  if (rc != TURBO_OK || !runtime || delivery_id == 0u) return TURBO_EINVAL;
  for (size_t i = 0u; i < runtime->config.max_sessions; ++i) {
  ...
  if (session->state == TURBO_FLOW_PROTOCOL_SESSION_DRAINING) {
    flow_protocol_runtime_session_release(runtime, session,
                                          status == TURBO_OK ? TURBO_ESHUTDOWN : status);
    return TURBO_OK;
  }
  if (status != TURBO_OK) {
...
int turbo_flow_protocol_runtime_session_close(turbo_flow_protocol_runtime_t *runtime,
                                              ...
                                              int status) {
  flow_protocol_session_t *session;
  if (!runtime || session_id == 0u || generation == 0u) return TURBO_EINVAL;
  session = flow_protocol_runtime_session_find(runtime, session_id);
  if (!session) return TURBO_ENOENT;
  if (session->generation != generation) return TURBO_EPROTO;
...
int turbo_flow_protocol_runtime_begin_shutdown(turbo_flow_protocol_runtime_t *runtime) {
  ...
  for (size_t i = 0u; i < runtime->config.max_sessions; ++i) {
    ...
    if (session->pending_delivery_id != 0u) {
      ...
      session->buffered = 0u;
      flow_protocol_runtime_session_release(runtime, session, TURBO_ESHUTDOWN);
    }
  }
