...
struct turbo_flow_protocol_business_registry_s {
  flow_protocol_business_entry_t *entries;
  size_t count;
  size_t capacity;
};
...
static int
flow_protocol_api_validate(const turbo_flow_protocol_plugin_api_t *api) {
  ...
  if (!api || api->size < sizeof(*api) ||
      api->version_major != TURBO_FLOW_PROTOCOL_PLUGIN_API_VERSION_MAJOR ||
      !api->name || !api->name[0] ||
...
static flow_protocol_entry_t *flow_protocol_registry_entry(
  ...
  for (size_t i = 0u; i < registry->count; ++i) {
    if (strcmp(registry->entries[i].api->name, name) == 0)
      return &registry->entries[i];
  }
  return NULL;
}

static void flow_protocol_service_cleanup(
    flow_protocol_entry_t *entry, turbo_flow_protocol_service_t *service) {
...
static void flow_protocol_reason_write(char *reason, size_t reason_size,
                                      const char *format,
                                      const char *detail) {
...
static flow_protocol_module_t flow_protocol_module_open(const char *path) {
#ifdef _WIN32
...
static void flow_protocol_module_close(flow_protocol_module_t module) {
  if (!module) return;
#ifdef _WIN32
...
static void *flow_protocol_module_symbol(flow_protocol_module_t module,
                                        const char *name) {
...
static const char *flow_protocol_module_error(char *buffer,
                                             size_t buffer_size) {
...
int turbo_flow_protocol_registry_create(
...
int turbo_flow_protocol_registry_destroy(
  ...
  while (registry->count > 0u) {
    ...
    flow_protocol_module_close(entry->module);
...
int turbo_flow_protocol_registry_load(
    ...
    size_t reason_size) {
  flow_protocol_module_t module;
  turbo_flow_protocol_plugin_get_api_fn get_api = NULL;
  const turbo_flow_protocol_plugin_api_t *api;
...
const turbo_flow_protocol_plugin_api_t *turbo_flow_protocol_registry_find(
    const turbo_flow_protocol_registry_t *registry, const char *name) {
  if (!registry || !name || !name[0]) return NULL;
  for (size_t i = 0u; i < registry->count; ++i) {
    if (strcmp(registry->entries[i].api->name, name) == 0)
...
int turbo_flow_protocol_owner_create_registered(
    turbo_flow_protocol_registry_t *registry, const char *name,
    const turbo_flow_protocol_open_request_t *request,
  ...
  if (service.size < sizeof(service) ||
      service.abi_version != TURBO_FLOW_PROTOCOL_ABI_VERSION ||
...
static int flow_protocol_business_api_validate(
    const turbo_flow_protocol_business_plugin_api_t *api) {
...
static void flow_protocol_business_service_cleanup(
    flow_protocol_business_entry_t *entry,
    turbo_flow_protocol_business_service_t *service) {
  if (!entry || !entry->api || !entry->api->close || !service ||
      (!service->instance && !service->owner))
...
int turbo_flow_protocol_business_registry_create(
    size_t capacity, turbo_flow_protocol_business_registry_t **out) {
  turbo_flow_protocol_business_registry_t *registry;
  if (!out || capacity == 0u ||
...
int turbo_flow_protocol_business_registry_load(
    turbo_flow_protocol_business_registry_t *registry, const char *path,
    char *reason, size_t reason_size) {
  flow_protocol_module_t module;
  turbo_flow_protocol_business_plugin_get_api_fn get_api = NULL;
  const turbo_flow_protocol_business_plugin_api_t *api;
  char module_error[256];
  void *symbol;
  int rc;
...
turbo_flow_protocol_business_registry_find(
    const turbo_flow_protocol_business_registry_t *registry,
    const char *business) {
  if (!registry || !business || !business[0]) return NULL;
  for (size_t i = 0u; i < registry->count; ++i) {
    if (strcmp(registry->entries[i].api->business, business) == 0)
...
int turbo_flow_protocol_business_owner_create_registered(
  ...
  turbo_flow_protocol_business_service_t service =
      TURBO_FLOW_PROTOCOL_BUSINESS_SERVICE_INIT;
  turbo_flow_protocol_business_info_t info =
      TURBO_FLOW_PROTOCOL_BUSINESS_INFO_INIT;
...
int turbo_flow_protocol_business_owner_instance(
    const turbo_flow_protocol_business_owner_t *owner,
    turbo_flow_protocol_kind_t expected_protocol,
