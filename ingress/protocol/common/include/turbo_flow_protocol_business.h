...
TURBO_FLOW_C_API int turbo_flow_protocol_business_command_view(
    const turbo_flow_protocol_business_command_output_t *output,
    turbo_flow_protocol_command_view_t *view);

TURBO_FLOW_C_API int
turbo_flow_protocol_business_get_info(const turbo_flow_protocol_business_t *business,
...
/**
 * Stable business provider descriptor registered through the unified root DLL
 * vtable. Concrete domain operations stay behind the provider-neutral instance.
 */
...
/** Creation from a retained unified catalog is declared in turbo_flow_plugin_protocol.h. */
TURBO_FLOW_C_API int
