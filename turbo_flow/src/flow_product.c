...
static int flow_product_runtime_integer(const json_value_t *value, size_t maximum,
  ...
  double number;
  size_t converted;
  if (!value || !result || json_type(value) != JSON_NUMBER) return SALTS_EPROTO;
  number = json_number(value);
  if (!isfinite(number) || number < 1.0 || number > (double)maximum) return SALTS_EPROTO;
...
int turbo_flow_resolved_config_runtime_ingress(const turbo_flow_resolved_config_t *config,
  ...
  capacity = ingress_value ? json_object_get(ingress_value, "capacity") : NULL;
  max_message_bytes =
      ingress_value ? json_object_get(ingress_value, "max_message_bytes") : NULL;
  max_inflight_bytes =
      ingress_value ? json_object_get(ingress_value, "max_inflight_bytes") : NULL;
  if (!runtime || json_type(runtime) != JSON_OBJECT || !ingress_value ||
      json_type(ingress_value) != JSON_OBJECT ||
  ...
  if (flow_product_runtime_integer(max_message_bytes,
                                   TURBO_FLOW_ASYNC_INGRESS_MAX_MESSAGE_BYTES, &value) != SALTS_OK)
    return SALTS_EPROTO;
  resolved.max_message_bytes = value;
  if (flow_product_runtime_integer(max_inflight_bytes,
                                   TURBO_FLOW_ASYNC_INGRESS_MAX_INFLIGHT_BYTES, &value) != SALTS_OK)
    return SALTS_EPROTO;
...
static int flow_product_registry_validate(const turbo_flow_product_provider_registry_t *registry,
  ...
  if (!registry || registry->size < sizeof(*registry) ||
    ...
    return flow_product_error(error, SALTS_EINVAL, "$.providers",
  ...
  for (size_t i = 0u; i < registry->adapter_provider_count; ++i) {
    ...
    if (provider->size < sizeof(*provider) || !provider->kind || !provider->kind[0] ||
      ...
      return flow_product_error(error, SALTS_EINVAL, "$.providers.adapters",
    ...
    for (size_t previous = 0u; previous < i; ++previous) {
      if (strcmp(provider->kind, registry->adapter_providers[previous].kind) == 0) {
        return flow_product_error(error, SALTS_EALREADY, "$.providers.adapters",
  ...
  for (size_t i = 0u; i < registry->resource_provider_count; ++i) {
    ...
    if (provider->size < sizeof(*provider) || !provider->kind || !provider->kind[0] ||
      ...
      return flow_product_error(error, SALTS_EINVAL, "$.providers.resources",
                               "invalid resource provider");
    }
    for (size_t previous = 0u; previous < i; ++previous) {
      if (strcmp(provider->kind, registry->resource_providers[previous].kind) == 0) {
        return flow_product_error(error, SALTS_EALREADY, "$.providers.resources",
...
int turbo_flow_product_preflight(const turbo_flow_resolved_config_t *config,
  ...
  int rc;
  if (!config || !config->document || !error || error->size < sizeof(*error)) return SALTS_EINVAL;
  *error = (turbo_flow_config_error_t)TURBO_FLOW_CONFIG_ERROR_INIT;
...
static int flow_product_channel_kind(const turbo_flow_resolved_config_t *config,
  ...
  channel = channels ? json_object_get(channels, resource_name) : NULL;
  (void)snprintf(path, sizeof(path), "$.channels.%s", resource_name);
  if (!channel || json_type(channel) != JSON_OBJECT)
    return flow_product_error(error, SALTS_ENOENT, path,
  ...
  if (!kind || json_type(kind) != JSON_STRING || !json_string(kind)[0])
    return flow_product_error(error, SALTS_EPROTO, path, "resolved channel kind is invalid");
  *kind_name = json_string(kind);
  return SALTS_OK;
}
...
int turbo_flow_product_assemble_graph(turbo_flow_t *flow,
  ...
  size_t stage_count;
  int rc;
  if (!flow || !config || !error || error->size < sizeof(*error)) return SALTS_EINVAL;
  if (turbo_flow_state(flow) != TURBO_FLOW_STATE_PARSED)
    return flow_product_error(error, SALTS_EINVAL, "$.graph",
  ...
  for (size_t i = 0u; i < stage_count; ++i) {
    ...
    if (!stage)
      return flow_product_error(error, SALTS_EPROTO, "$.graph", "invalid Graph stage");
  ...
  for (size_t i = 0u; i < stage_count; ++i) {
    ...
    *error = (turbo_flow_config_error_t)TURBO_FLOW_CONFIG_ERROR_INIT;
    ...
    if (rc != SALTS_OK) return flow_product_provider_error(error, rc, "channels", resource_name);
  }
  ...
  for (size_t i = 0u; i < stage_count; ++i) {
    ...
    rc = turbo_flow_resolved_config_adapter(config, adapter_name, &view);
    if (rc != SALTS_OK) return flow_product_provider_error(error, rc, "adapters", adapter_name);
