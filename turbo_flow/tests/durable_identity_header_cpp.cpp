...
static_assert(TURBO_FLOW_DURABLE_IDENTITY_API_VERSION == 2u,
              "durable identity ABI version must match its extended layout");
...
int turbo_flow_durable_identity_header_cpp_probe() {
  ...
  return identity.size == sizeof(identity) && identity.version == TURBO_FLOW_DURABLE_IDENTITY_API_VERSION && setter && getter ? 0 : 1;
}
