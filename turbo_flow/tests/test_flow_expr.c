...
static void check_eval_value_pair(const char *text, turbo_flow_expr_eval_context_t *context,
  ...
  case TURBO_FLOW_EXPR_TYPE_BOOL:
    check_equal(interp_value.as.boolean, expected->as.boolean);
    if (jit) check_equal(jit_value.as.boolean, interp_value.as.boolean);
    break;
  case TURBO_FLOW_EXPR_TYPE_I64:
    check_equal(interp_value.as.i64, expected->as.i64);
    if (jit) check_equal(jit_value.as.i64, interp_value.as.i64);
    break;
  case TURBO_FLOW_EXPR_TYPE_F64:
    check_within(interp_value.as.f64, expected->as.f64, 1e-12);
    if (jit) check_within(jit_value.as.f64, interp_value.as.f64, 1e-12);
    break;
  case TURBO_FLOW_EXPR_TYPE_STRING:
    check_equal(interp_value.as.string.len, expected->as.string.len);
    check_equal(interp_value.as.string.data, expected->as.string.data, expected->as.string.len);
    if (jit) {
      check_equal(jit_value.as.string.len, interp_value.as.string.len);
      check_equal(jit_value.as.string.data, interp_value.as.string.data,
...
spec("flow_expr") {
  ...
  it("resolves built-in fields and computes predicate types") {
    ...
    field = expr_node(&ast, left->left, FLOW_EXPR_FIELD);
    check_equal(field->value_type, FLOW_EXPR_TYPE_I64);
    check_equal(field->field_id, FLOW_EXPR_FIELD_MSG_STATUS);
    right = expr_node(&ast, root->right, FLOW_EXPR_NE);
    field = expr_node(&ast, right->left, FLOW_EXPR_FIELD);
    check_equal(field->value_type, FLOW_EXPR_TYPE_STRING);
    check_equal(field->field_id, FLOW_EXPR_FIELD_MSG_PAYLOAD);
    flow_expr_ast_destroy(&ast);
  }
  ...
  it("resolves schema fields and promotes mixed numeric arithmetic") {
    ...
    check_equal(add->value_type, FLOW_EXPR_TYPE_F64);
    ...
    check_equal(field->field_id, 101);
    flow_expr_ast_destroy(&ast);
  }
  ...
  it("reads built-in and schema fields through the read-only evaluation ABI") {
    ...
    msg.payload = vstr_from_buf("body", 4);
    context.message = &msg;
  ...
  it("keeps MIR interpreter and JIT scalar semantics identical") {
    ...
    expected.as.string = vstr_from_cstr("literal");
  ...
  it("resolves AUTO once and validates backend options") {
    ...
    turbo_flow_expr_destroy(expr);
    check_equal(turbo_flow_expr_backend(NULL), TURBO_FLOW_EXPR_BACKEND_INVALID);
  ...
  it("bounds MIR evaluation depth and tears down repeated programs") {
    ...
    check_equal(turbo_vec_init(&depth_expr.ast.nodes, sizeof(flow_expr_node_t)), TURBO_OK);
    ...
    node.i64 = 1;
    check_equal(turbo_vec_push(&depth_expr.ast.nodes, &node), TURBO_OK);
    root = 0;
