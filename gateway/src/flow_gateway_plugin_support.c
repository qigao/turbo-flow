...
static int flow_gateway_version_supported(
    const flow_gateway_plugin_descriptor_t *descriptor, const char *version) {
...
int flow_gateway_plugin_open(
  ...
  const flow_gateway_plugin_descriptor_t *descriptor =
      (const flow_gateway_plugin_descriptor_t *)ctx;
  ...
  if (!flow_gateway_version_supported(descriptor, version))
    return TURBO_ENOTSUP;
  ops.inspect = descriptor->inspect;
  ops.reply = descriptor->reply;
  ops.encode = descriptor->encode;
  if (ops.reply) capabilities |= TURBO_FLOW_GATEWAY_CAP_PROTOCOL_REPLY;
  if (ops.encode) capabilities |= TURBO_FLOW_GATEWAY_CAP_COMMAND_ENCODE;
...
int flow_gateway_metadata_text(char *out, size_t capacity,
                               const char *text) {
  const size_t length = text ? strlen(text) : 0u;
  return flow_gateway_metadata_text_n(out, capacity, text, length);
}
...
static uint8_t flow_gateway_coap_response_code(uint8_t request_code,
                                               int status) {
...
int flow_gateway_coap_reply(
  ...
  output->data[1] = flow_gateway_coap_response_code(code, status);
  if (token_length > 0u)
...
static int flow_gateway_coap_code(const char *operation, int lwm2m,
                                  uint8_t *out) {
...
static int flow_gateway_coap_option_write(uint8_t *data, size_t capacity,
...
int flow_gateway_coap_encode(
  ...
  if (!command || !output || !output->data || !command->resource)
    return TURBO_EINVAL;
  rc = flow_gateway_coap_code(command->operation, lwm2m, &code);
  if (rc != TURBO_OK) return rc;
  ...
  for (;;) {
    ...
    rc = flow_gateway_coap_option_write(
