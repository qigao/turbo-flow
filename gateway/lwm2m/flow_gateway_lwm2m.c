...
static int flow_lwm2m_inspect(
  ...
  return flow_gateway_coap_inspect(frame, metadata, 1);
}

static int flow_lwm2m_reply(
    void *ctx, const char *configured_version,
  ...
  if ((uint16_t)next == 0u)
    next = atomic_fetch_add_explicit((atomic_uint *)ctx, 1u,
                                     memory_order_relaxed) +
           1u;
  return flow_gateway_coap_reply(request, status, output, 1,
                                 (uint16_t)next);
}

static int flow_lwm2m_encode(
    void *ctx, const char *configured_version,
    const turbo_flow_gateway_command_view_t *command,
    turbo_flow_gateway_frame_output_t *output) {
  (void)ctx;
  (void)configured_version;
  return flow_gateway_coap_encode(command, output, 1);
}

static const char *const FLOW_LWM2M_VERSIONS[] = {"1.2.2"};
static const flow_gateway_plugin_descriptor_t FLOW_LWM2M_DESCRIPTOR = {
    "lwm2m", TURBO_FLOW_GATEWAY_PROTOCOL_LWM2M, "1.2.2",
    FLOW_LWM2M_VERSIONS,
    sizeof(FLOW_LWM2M_VERSIONS) / sizeof(FLOW_LWM2M_VERSIONS[0]),
    flow_lwm2m_inspect, flow_lwm2m_reply, flow_lwm2m_encode,
    &FLOW_LWM2M_NEXT_MESSAGE_ID};

static const turbo_flow_gateway_plugin_api_t FLOW_LWM2M_API = {
