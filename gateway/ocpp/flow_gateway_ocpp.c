...
static int flow_ocpp_inspect(void *ctx, const char *configured_version,
  ...
  if (turbo_json_string_len(correlation_value) == 0u ||
      turbo_json_string_len(correlation_value) > TURBO_FLOW_GATEWAY_CORRELATION_MAX)
    goto done;
  if (flow_gateway_metadata_text_n(metadata->correlation_id, sizeof(metadata->correlation_id),
  ...
  if (message_type == 2u) {
    ...
    if (!operation_value || turbo_json_type(operation_value) != TURBO_JSON_STRING ||
        ...
        turbo_json_string_len(operation_value) > TURBO_FLOW_GATEWAY_OPERATION_MAX)
      goto done;
  ...
  if (message_type == 2u)
    rc = flow_gateway_metadata_text_n(metadata->operation, sizeof(metadata->operation), operation,
                                      turbo_json_string_len(operation_value));
  else rc = flow_gateway_metadata_text(metadata->operation, sizeof(metadata->operation), operation);
...
static const turbo_flow_gateway_plugin_api_t FLOW_OCPP_API = {
    ...
    TURBO_FLOW_GATEWAY_CAP_INGRESS | TURBO_FLOW_GATEWAY_CAP_EGRESS |
        TURBO_FLOW_GATEWAY_CAP_RAW_PRESERVE | TURBO_FLOW_GATEWAY_CAP_PROTOCOL_REPLY |
        TURBO_FLOW_GATEWAY_CAP_COMMAND_ENCODE,
    (void *)&FLOW_OCPP_DESCRIPTOR,
    flow_gateway_plugin_open,
    flow_gateway_plugin_close};

const turbo_flow_gateway_plugin_api_t *turbo_flow_gateway_plugin_get_api(void) {
