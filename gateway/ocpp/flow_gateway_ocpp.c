...
static int flow_ocpp_action_supported(const char *version, const char *action,
                                      size_t action_size) {
  static const char *const v16[] = {
      "Authorize",          "BootNotification",  "CancelReservation",
      "ChangeAvailability", "ChangeConfiguration", "ClearCache",
      "ClearChargingProfile", "DataTransfer",    "DiagnosticsStatusNotification",
      "FirmwareStatusNotification", "GetCompositeSchedule", "GetConfiguration",
      "GetDiagnostics",     "GetLocalListVersion", "Heartbeat",
      "MeterValues",        "RemoteStartTransaction", "RemoteStopTransaction",
      "ReserveNow",         "Reset",             "SendLocalList",
      "SetChargingProfile", "StartTransaction",  "StatusNotification",
      "StopTransaction",    "TriggerMessage",    "UnlockConnector",
      "UpdateFirmware"};
  static const char *const v201[] = {
      "Authorize", "BootNotification", "CancelReservation",
      "CertificateSigned", "ChangeAvailability", "ClearCache",
      "ClearChargingProfile", "ClearDisplayMessage", "ClearedChargingLimit",
      "CostUpdated", "CustomerInformation", "DataTransfer", "DeleteCertificate",
      "FirmwareStatusNotification", "Get15118EVCertificate",
      "GetBaseReport", "GetCertificateStatus", "GetChargingProfiles",
      ...
      "GetTransactionStatus", "GetVariables", "Heartbeat", "InstallCertificate",
      ...
      "NotifyReport", "PublishFirmware", "PublishFirmwareStatusNotification",
      "ReportChargingProfiles", "RequestStartTransaction", "RequestStopTransaction",
      "ReservationStatusUpdate", "ReserveNow", "Reset", "SecurityEventNotification",
      "SendLocalList", "SetChargingProfile", "SetDisplayMessage",
      "SetMonitoringBase", "SetMonitoringLevel", "SetNetworkProfile",
      "SetVariableMonitoring", "SetVariables", "SignCertificate",
      "StatusNotification", "TransactionEvent", "TriggerMessage",
      "UnlockConnector", "UnpublishFirmware", "UpdateFirmware"};
  ...
  for (size_t i = 0u; i < count; ++i)
    if (strlen(actions[i]) == action_size &&
        memcmp(actions[i], action, action_size) == 0)
      return 1;
  return 0;
}

static int flow_ocpp_inspect(
  ...
  (void)ctx;
  if (!frame || !metadata ||
      turbo_parse_json(frame->data, frame->data_size, &document) != TURBO_OK ||
  ...
  if (!message_type_value ||
      turbo_json_type(message_type_value) != TURBO_JSON_NUMBER ||
      !correlation_value ||
      turbo_json_type(correlation_value) != TURBO_JSON_STRING)
    goto done;
  message_type_number = turbo_json_number(message_type_value);
  if (!isfinite(message_type_number) ||
      message_type_number != floor(message_type_number) ||
  ...
  if (turbo_json_string_len(correlation_value) == 0u ||
      turbo_json_string_len(correlation_value) >
          TURBO_FLOW_GATEWAY_CORRELATION_MAX)
    goto done;
  if (flow_gateway_metadata_text_n(
          metadata->correlation_id, sizeof(metadata->correlation_id),
  ...
  if (message_type == 2u) {
    ...
    if (!operation_value ||
        turbo_json_type(operation_value) != TURBO_JSON_STRING ||
        turbo_json_string_len(operation_value) == 0u ||
        turbo_json_string_len(operation_value) >
            TURBO_FLOW_GATEWAY_OPERATION_MAX)
      goto done;
    if (!flow_ocpp_action_supported(
            configured_version, turbo_json_string(operation_value),
            turbo_json_string_len(operation_value)))
      goto done;
    if (!turbo_json_array_get(document, 3u) ||
        turbo_json_type(turbo_json_array_get(document, 3u)) !=
            TURBO_JSON_OBJECT)
      goto done;
    operation = turbo_json_string(operation_value);
  } else if (message_type == 3u) {
    if (turbo_json_array_size(document) != 3u ||
        !turbo_json_array_get(document, 2u) ||
        turbo_json_type(turbo_json_array_get(document, 2u)) !=
            TURBO_JSON_OBJECT)
      goto done;
    operation = "call-result";
  } else {
    if (turbo_json_array_size(document) != 5u ||
        !turbo_json_array_get(document, 2u) ||
        turbo_json_type(turbo_json_array_get(document, 2u)) !=
            TURBO_JSON_STRING ||
        ...
        turbo_json_type(turbo_json_array_get(document, 4u)) !=
            TURBO_JSON_OBJECT)
      goto done;
  ...
  if (message_type == 2u)
    rc = flow_gateway_metadata_text_n(
        metadata->operation, sizeof(metadata->operation), operation,
        turbo_json_string_len(operation_value));
  else
    rc = flow_gateway_metadata_text(metadata->operation,
                                    sizeof(metadata->operation), operation);
...
static int flow_ocpp_write(flow_ocpp_writer_t *writer, const void *data,
                           size_t size) {
...
static int flow_ocpp_write_string(flow_ocpp_writer_t *writer,
...
static int flow_ocpp_payload_validate(const uint8_t *payload,
                                      size_t payload_size) {
  turbo_json_doc_t *document = NULL;
  int rc = TURBO_EPROTO;
  if (!payload || payload_size == 0u) return TURBO_EINVAL;
  if (turbo_parse_json(payload, payload_size, &document) == TURBO_OK &&
...
static int flow_ocpp_encode_envelope(
    unsigned message_type, const char *correlation_id,
  ...
  int rc;
  if (!correlation_id || !correlation_id[0] || !output || !output->data)
    return TURBO_EINVAL;
  ...
  writer.size = 0u;
  rc = flow_ocpp_write(&writer, message_type == 2u ? "[2," :
                                    message_type == 3u ? "[3," : "[4,",
                       3u);
  if (rc == TURBO_OK)
    rc = flow_ocpp_write_string(&writer, correlation_id,
                                strlen(correlation_id));
  if (rc == TURBO_OK && message_type == 2u) {
    rc = flow_ocpp_write(&writer, ",", 1u);
    if (rc == TURBO_OK)
      rc = flow_ocpp_write_string(&writer, operation, strlen(operation));
    ...
    if (rc == TURBO_OK)
      rc = flow_ocpp_write_string(
          &writer, error_description ? error_description : "",
          error_description ? strlen(error_description) : 0u);
  }
  if (rc == TURBO_OK) rc = flow_ocpp_write(&writer, ",", 1u);
  if (rc == TURBO_OK)
    rc = flow_ocpp_write(&writer, payload, payload_size);
  if (rc == TURBO_OK) rc = flow_ocpp_write(&writer, "]", 1u);
  if (rc != TURBO_OK) return rc;
...
static int flow_ocpp_reply(
  ...
  if (!request || !output ||
      turbo_parse_json(request->data, request->data_size, &document) !=
          TURBO_OK ||
      !document || turbo_json_type(document) != TURBO_JSON_ARRAY)
    goto done;
  type_value = turbo_json_array_get(document, 0u);
  correlation = turbo_json_array_get(document, 1u);
  if (!type_value || turbo_json_type(type_value) != TURBO_JSON_NUMBER ||
      !correlation || turbo_json_type(correlation) != TURBO_JSON_STRING)
    goto done;
  ...
  rc = flow_ocpp_encode_envelope(
      status == TURBO_OK ? 3u : 4u, turbo_json_string(correlation),
...
static int flow_ocpp_encode(
  ...
  if (strcmp(command->operation, "call-result") == 0) {
    ...
    message_type = 4u;
    wire_operation =
        command->resource && command->resource[0] ? command->resource
                                                  : "GenericError";
  } else {
    wire_operation = command->operation;
    if (!flow_ocpp_action_supported(configured_version, wire_operation,
                                    strlen(wire_operation)))
      return TURBO_ENOTSUP;
  }
  return flow_ocpp_encode_envelope(
      message_type, command->correlation_id, wire_operation,
...
static const flow_gateway_plugin_descriptor_t FLOW_OCPP_DESCRIPTOR = {
    "ocpp", TURBO_FLOW_GATEWAY_PROTOCOL_OCPP, "2.0.1",
    FLOW_OCPP_VERSIONS,
    sizeof(FLOW_OCPP_VERSIONS) / sizeof(FLOW_OCPP_VERSIONS[0]),
    flow_ocpp_inspect, flow_ocpp_reply, flow_ocpp_encode, NULL};

static const turbo_flow_gateway_plugin_api_t FLOW_OCPP_API = {
