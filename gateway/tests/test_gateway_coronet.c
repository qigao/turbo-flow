...
#include <string.h>
  ...
  #include <winsock2.h>
  #include <ws2tcpip.h>
typedef SOCKET gateway_test_socket_t;
  #define GATEWAY_TEST_INVALID_SOCKET INVALID_SOCKET
#else
  #include <arpa/inet.h>
  #include <netinet/in.h>
  #include <sys/socket.h>
  #include <unistd.h>
typedef int gateway_test_socket_t;
  #define GATEWAY_TEST_INVALID_SOCKET (-1)
#endif

enum {
  GATEWAY_TEST_FRAME_CAPACITY = 128,
  GATEWAY_TEST_RECONNECT_ITERATIONS = 4,
  GATEWAY_TEST_WAIT_MS = 3000,
  GATEWAY_TEST_SHUTDOWN_WAIT_MS = 10000
};

typedef struct gateway_coronet_probe_s {
  atomic_int publishes;
  atomic_ullong delivery_id;
  atomic_int verified_identities;
  int pending;
} gateway_coronet_probe_t;

typedef struct gateway_ws_client_s {
...
static void gateway_coronet_probe_init(gateway_coronet_probe_t *probe) {
...
static int gateway_coronet_publish(void *ctx, const turbo_flow_gateway_publish_request_t *request,
                                   turbo_flow_gateway_publish_disposition_t *disposition) {
  gateway_coronet_probe_t *probe = (gateway_coronet_probe_t *)ctx;
  ...
  *disposition =
      probe->pending ? TURBO_FLOW_GATEWAY_PUBLISH_PENDING : TURBO_FLOW_GATEWAY_PUBLISH_SETTLED;
  return TURBO_OK;
}

static int gateway_coronet_identity(void *ctx,
                                    const turbo_flow_gateway_coronet_identity_request_t *request,
...
static int
gateway_coronet_mtls_identity(void *ctx,
                              const turbo_flow_gateway_coronet_identity_request_t *request,
                              char *device_id, size_t capacity) {
...
static int gateway_coronet_plugin_open(const char *module, const char *name,
  ...
  turbo_flow_gateway_open_request_t request = TURBO_FLOW_GATEWAY_OPEN_REQUEST_INIT;
  ...
  if (gateway_out) *gateway_out = NULL;
  if (!module || !name || !registry_out || !owner_out || !gateway_out) return TURBO_EINVAL;
  request.protocol = protocol;
  request.protocol_version = version;
  request.max_frame_size = GATEWAY_TEST_FRAME_CAPACITY;
  rc = turbo_flow_gateway_registry_create(1u, &registry);
  if (rc == TURBO_OK)
    rc = turbo_flow_gateway_registry_load(registry, module, reason, sizeof(reason));
  if (rc == TURBO_OK)
    rc = turbo_flow_gateway_owner_create_registered(registry, name, &request, &owner);
  if (rc == TURBO_OK) rc = turbo_flow_gateway_owner_instance(owner, protocol, &gateway);
  if (rc != TURBO_OK) {
...
static void gateway_coronet_plugin_close(turbo_flow_gateway_registry_t *registry,
                                         turbo_flow_gateway_owner_t *owner) {
  turbo_flow_gateway_owner_destroy(owner);
  if (registry) (void)turbo_flow_gateway_registry_destroy(registry);
}

static void gateway_coronet_config_init(turbo_flow_gateway_coronet_config_t *config,
                                        turbo_flow_gateway_t *gateway,
                                        turbo_flow_gateway_coronet_transport_t transport, int port,
                                        gateway_coronet_probe_t *probe) {
  *config = (turbo_flow_gateway_coronet_config_t)TURBO_FLOW_GATEWAY_CORONET_CONFIG_INIT;
  config->gateway = gateway;
  config->transport = transport;
  config->port = port;
  config->runtime.max_sessions = 4u;
  config->runtime.max_frame_size = GATEWAY_TEST_FRAME_CAPACITY;
  config->runtime.max_buffered_bytes =
      (config->runtime.max_sessions + 1u) * config->runtime.max_frame_size;
  config->runtime_ops.publish = gateway_coronet_publish;
  config->runtime_ctx = probe;
  config->resolve_identity = gateway_coronet_identity;
  config->identity_ctx = (void *)"device-1";
}

static void gateway_test_socket_close(gateway_test_socket_t socket_handle) {
  if (socket_handle == GATEWAY_TEST_INVALID_SOCKET) return;
#ifdef _WIN32
...
static unsigned short gateway_test_pick_port(int datagram) {
  ...
  if (socket_handle != GATEWAY_TEST_INVALID_SOCKET) {
    ...
    gateway_test_socket_close(socket_handle);
  }
#ifdef _WIN32
  WSACleanup();
#endif
...
static gateway_test_socket_t gateway_test_connect_tcp(unsigned short port) {
  struct sockaddr_in address;
  gateway_test_socket_t socket_handle = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
  if (socket_handle == GATEWAY_TEST_INVALID_SOCKET) return GATEWAY_TEST_INVALID_SOCKET;
  memset(&address, 0, sizeof(address));
  address.sin_family = AF_INET;
  ...
  if (connect(socket_handle, (const struct sockaddr *)&address, sizeof(address)) != 0) {
    gateway_test_socket_close(socket_handle);
    return GATEWAY_TEST_INVALID_SOCKET;
  }
  return socket_handle;
}

static int gateway_test_send_udp(unsigned short port, const uint8_t *data, size_t size) {
  struct sockaddr_in address;
  gateway_test_socket_t socket_handle = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
#ifdef _WIN32
  int sent;
#else
  ssize_t sent;
#endif
  if (socket_handle == GATEWAY_TEST_INVALID_SOCKET) return TURBO_EIO;
  memset(&address, 0, sizeof(address));
  address.sin_family = AF_INET;
  address.sin_port = htons(port);
  address.sin_addr.s_addr = htonl(INADDR_LOOPBACK);
...
static int gateway_test_send_tcp(gateway_test_socket_t socket_handle, const uint8_t *data,
                                 size_t size) {
#ifdef _WIN32
...
static int gateway_test_wait_socket_readable(gateway_test_socket_t socket_handle,
                                             unsigned timeout_ms) {
  fd_set read_set;
  struct timeval timeout;
  int ready;
  if (socket_handle == GATEWAY_TEST_INVALID_SOCKET) return TURBO_EINVAL;
  FD_ZERO(&read_set);
...
static int gateway_test_recv_exact(gateway_test_socket_t socket_handle, uint8_t *data,
                                   size_t size) {
  size_t received = 0u;
...
static int gateway_test_wait_publishes(gateway_coronet_probe_t *probe, int expected) {
  const uint64_t deadline = turbo_monotonic_ms() + GATEWAY_TEST_WAIT_MS;
...
static int gateway_test_wait_quiescent(turbo_flow_gateway_coronet_server_t *server,
  ...
  do {
    snapshot = (turbo_flow_gateway_coronet_snapshot_t)TURBO_FLOW_GATEWAY_CORONET_SNAPSHOT_INIT;
    rc = turbo_flow_gateway_coronet_server_snapshot(server, &snapshot);
    if (rc != TURBO_OK) return rc;
...
static void gateway_test_gbt_frame(uint8_t frame[25]) {
...
static void gateway_ws_client_task(coro_t *coroutine, void *arg) {
  gateway_ws_client_t *client = (gateway_ws_client_t *)arg;
  ...
  coro_socket_set_timeout(socket_handle, GATEWAY_TEST_WAIT_MS);
  if (client->secure) {
...
static int gateway_test_run_coronet_client(gateway_ws_client_t *client) {
  coro_context_t *context;
  int rc;
  ...
  client->done = 0;
  rc = coro_context_spawn(context, gateway_ws_client_task, client);
  if (rc == TURBO_OK) rc = coro_context_run(context, TURBO_RUN_DEFAULT);
  coro_context_destroy(context);
  return rc;
}

static void gateway_test_coronet_client_thread(void *arg) {
  gateway_ws_client_t *client = (gateway_ws_client_t *)arg;
  if (client) client->run_status = gateway_test_run_coronet_client(client);
}

static int gateway_test_coronet_server_cleanup(turbo_flow_gateway_coronet_server_t *server) {
  int rc;
  if (!server) return TURBO_OK;
  rc = turbo_flow_gateway_coronet_server_destroy(server);
  if (rc != TURBO_EBUSY) return rc;
  rc = turbo_flow_gateway_coronet_server_begin_shutdown(server);
  if (rc == TURBO_OK)
    rc = turbo_flow_gateway_coronet_server_wait_shutdown(server, GATEWAY_TEST_SHUTDOWN_WAIT_MS);
  if (rc == TURBO_ETIMEDOUT)
    rc = turbo_flow_gateway_coronet_server_force_shutdown(
        server, TURBO_ECANCELED, GATEWAY_TEST_SHUTDOWN_WAIT_MS);
  if (rc == TURBO_OK) rc = turbo_flow_gateway_coronet_server_destroy(server);
  return rc;
}

typedef enum gateway_test_mtls_fault_e {
  GATEWAY_TEST_MTLS_NO_FAULT = 0,
  GATEWAY_TEST_MTLS_FAIL_AFTER_PLUGIN_OPEN
} gateway_test_mtls_fault_t;

static int gateway_test_wss_mtls_identity_trace(gateway_test_mtls_fault_t fault) {
  static const char payload[] = "[2,\"84\",\"BootNotification\",{}]";
  char cert_file[512] = {0};
  char key_file[512] = {0};
  gateway_coronet_probe_t probe;
  gateway_ws_client_t missing_client;
  gateway_ws_client_t verified_client;
  turbo_flow_gateway_registry_t *registry = NULL;
  turbo_flow_gateway_owner_t *owner = NULL;
  turbo_flow_gateway_t *gateway = NULL;
  turbo_flow_gateway_coronet_server_t *server = NULL;
  turbo_flow_gateway_coronet_config_t config;
  const unsigned short port = gateway_test_pick_port(0);
  int cleanup_rc;
  int rc = TURBO_OK;

  gateway_coronet_probe_init(&probe);
  ...
  rc = gateway_coronet_plugin_open(FLOW_GATEWAY_OCPP_MODULE, "ocpp",
                                   TURBO_FLOW_GATEWAY_PROTOCOL_OCPP, "2.0.1", &registry, &owner,
                                   &gateway);
  if (rc != TURBO_OK) goto cleanup;
  if (fault == GATEWAY_TEST_MTLS_FAIL_AFTER_PLUGIN_OPEN) {
    rc = TURBO_ECANCELED;
    goto cleanup;
  }

  gateway_coronet_config_init(&config, gateway, TURBO_FLOW_GATEWAY_CORONET_WSS, port, &probe);
  config.path = "/ocpp";
  ...
  config.resolve_identity = gateway_coronet_mtls_identity;
  config.identity_ctx = &probe;
  rc = turbo_flow_gateway_coronet_server_create(&config, &server);
  if (rc != TURBO_OK) goto cleanup;
  rc = turbo_flow_gateway_coronet_server_start(server);
  if (rc != TURBO_OK) goto cleanup;
  ...
  rc = gateway_test_run_coronet_client(&missing_client);
  if (rc != TURBO_OK) goto cleanup;
  ...
  rc = gateway_test_run_coronet_client(&verified_client);
  if (rc != TURBO_OK) goto cleanup;
  if (!verified_client.done || verified_client.status != TURBO_OK) {
    rc = TURBO_EPROTO;
    goto cleanup;
  }
  rc = gateway_test_wait_publishes(&probe, 1);
  if (rc != TURBO_OK) goto cleanup;
...
cleanup:
  if (server) {
    cleanup_rc = gateway_test_coronet_server_cleanup(server);
    if (rc == TURBO_OK && cleanup_rc != TURBO_OK) rc = cleanup_rc;
  }
  gateway_coronet_plugin_close(registry, owner);
  tls_test_remove_file(key_file);
  tls_test_remove_file(cert_file);
  return rc;
}

static int gateway_test_wss_mtls_reconnect_trace(void) {
  ...
  for (iteration = 0; iteration < GATEWAY_TEST_RECONNECT_ITERATIONS; ++iteration) {
    gateway_ws_client_t client;
    ...
    rc = turbo_thread_create(&client_thread, gateway_test_coronet_client_thread, &client);
    if (rc != TURBO_OK) goto cleanup;
    stage = "publish";
    rc = gateway_test_wait_publishes(&probe, iteration + 1);
    ...
    stage = "quiescence";
    rc = gateway_test_wait_quiescent(server, &snapshot);
    if (rc != TURBO_OK) goto cleanup;
    if (snapshot.active_handlers != 0u || snapshot.runtime.active_sessions != 0u ||
        snapshot.runtime.pending_settlements != 0u || snapshot.runtime.buffered_bytes != 0u ||
  ...
  if (atomic_load_explicit(&probe.publishes, memory_order_acquire) !=
      GATEWAY_TEST_RECONNECT_ITERATIONS) {
    rc = TURBO_EPROTO;
    goto cleanup;
  }
...
cleanup:
  ...
  if (cleanup_rc != TURBO_OK) {
    fprintf(stderr,
            "GATEWAY_TRACE_CLEANUP_FAILURE trace=wss-mtls-reconnect stage=%s "
  ...
  if (rc != TURBO_OK)
    fprintf(stderr,
            "GATEWAY_TRACE_FAILURE trace=wss-mtls-reconnect stage=%s iteration=%d status=%d\n",
            stage, iteration, rc);
  return rc;
}

static int gateway_test_tcp_settlement_shutdown_trace(void) {
  uint8_t frame[25];
  uint8_t reply[25];
  gateway_coronet_probe_t probe;
  turbo_flow_gateway_registry_t *registry = NULL;
  ...
  gateway_test_socket_t client = GATEWAY_TEST_INVALID_SOCKET;
  const unsigned short port = gateway_test_pick_port(0);
  const char *stage = "pick-port";
  ...
  stage = "plugin-open";
  rc = gateway_coronet_plugin_open(
      FLOW_GATEWAY_GBT32960_MODULE, "gbt32960",
      TURBO_FLOW_GATEWAY_PROTOCOL_GBT_32960, "2025", &registry, &owner,
      &gateway);
  if (rc != TURBO_OK) goto cleanup;
  gateway_coronet_config_init(&config, gateway,
                              TURBO_FLOW_GATEWAY_CORONET_TCP, port, &probe);
  ...
  client = gateway_test_connect_tcp(port);
  if (client == GATEWAY_TEST_INVALID_SOCKET) {
    rc = TURBO_ECONNREFUSED;
    goto cleanup;
  }
  stage = "client-send";
  rc = gateway_test_send_tcp(client, frame, sizeof(frame));
  if (rc != TURBO_OK) goto cleanup;
  stage = "publish";
  rc = gateway_test_wait_publishes(&probe, 1);
  if (rc != TURBO_OK) goto cleanup;
  ...
  rc = gateway_test_wait_socket_readable(client, 20u);
  if (rc != TURBO_ETIMEDOUT) {
  ...
  rc = turbo_flow_gateway_coronet_server_wait_shutdown(server, 20u);
  if (rc != TURBO_ETIMEDOUT) {
    if (rc == TURBO_OK) rc = TURBO_EPROTO;
    goto cleanup;
  }
  stage = "settle";
  rc = turbo_flow_gateway_coronet_server_settle(server, delivery_id,
                                                TURBO_OK, &result);
  if (rc != TURBO_OK) goto cleanup;
  stage = "reply-ready";
  rc = gateway_test_wait_socket_readable(client, GATEWAY_TEST_WAIT_MS);
  if (rc != TURBO_OK) goto cleanup;
  stage = "reply-receive";
  rc = gateway_test_recv_exact(client, reply, sizeof(reply));
  if (rc != TURBO_OK) goto cleanup;
  ...
  rc = turbo_flow_gateway_coronet_server_wait_shutdown(
      server, GATEWAY_TEST_WAIT_MS);

cleanup:
  if (client != GATEWAY_TEST_INVALID_SOCKET)
    gateway_test_socket_close(client);
  cleanup_rc = gateway_test_coronet_server_cleanup(server);
  if (cleanup_rc != TURBO_OK) {
    fprintf(stderr,
            "GATEWAY_TRACE_CLEANUP_FAILURE "
  ...
  if (rc != TURBO_OK)
    fprintf(stderr,
            "GATEWAY_TRACE_FAILURE trace=tcp-settlement-shutdown "
            "stage=%s status=%d delivery_id=%" PRIu64 "\n",
            stage, rc, delivery_id);
  return rc;
}

spec("gateway CoroNet transport owner") {
  it("rejects incompatible transports and implicit LwM2M downgrade") {
    ...
    turbo_flow_gateway_coronet_config_t config;
    gateway_coronet_probe_init(&probe);
    check_int_eq(gateway_coronet_plugin_open(FLOW_GATEWAY_LWM2M_MODULE, "lwm2m",
                                             TURBO_FLOW_GATEWAY_PROTOCOL_LWM2M, "1.2.2", &registry,
                                             &owner, &gateway),
                 TURBO_OK);
    gateway_coronet_config_init(&config, gateway, TURBO_FLOW_GATEWAY_CORONET_UDP, 0, &probe);
    check_int_eq(turbo_flow_gateway_coronet_server_create(&config, &server), TURBO_EPERM);
    check_null(server);
    config.allow_insecure_lwm2m = 1u;
    check_int_eq(turbo_flow_gateway_coronet_server_create(&config, &server), TURBO_OK);
    check_int_eq(turbo_flow_gateway_coronet_server_destroy(server), TURBO_OK);
    gateway_coronet_plugin_close(registry, owner);
  }
  ...
  it("maps one CoAP datagram on a bounded UDP session") {
    ...
    gateway_coronet_probe_t probe;
    turbo_flow_gateway_coronet_snapshot_t snapshot =
        TURBO_FLOW_GATEWAY_CORONET_SNAPSHOT_INIT;
    turbo_flow_gateway_registry_t *registry = NULL;
    turbo_flow_gateway_owner_t *owner = NULL;
    turbo_flow_gateway_t *gateway = NULL;
    turbo_flow_gateway_coronet_server_t *server = NULL;
    turbo_flow_gateway_coronet_config_t config;
    unsigned short port = gateway_test_pick_port(1);
    int shutdown_rc;
    gateway_coronet_probe_init(&probe);
    check_true(port != 0u);
    check_int_eq(gateway_coronet_plugin_open(FLOW_GATEWAY_COAP_MODULE, "coap",
                                             TURBO_FLOW_GATEWAY_PROTOCOL_COAP, "RFC7252", &registry,
                                             &owner, &gateway),
                 TURBO_OK);
    gateway_coronet_config_init(&config, gateway, TURBO_FLOW_GATEWAY_CORONET_UDP, port, &probe);
    check_int_eq(turbo_flow_gateway_coronet_server_create(&config, &server), TURBO_OK);
    check_int_eq(turbo_flow_gateway_coronet_server_start(server), TURBO_OK);
    check_int_eq(gateway_test_send_udp(port, frame, sizeof(frame)), TURBO_OK);
    check_int_eq(gateway_test_wait_publishes(&probe, 1), TURBO_OK);
    check_int_eq(turbo_flow_gateway_coronet_server_begin_shutdown(server), TURBO_OK);
    shutdown_rc =
        turbo_flow_gateway_coronet_server_wait_shutdown(server,
                                                        GATEWAY_TEST_WAIT_MS);
    if (shutdown_rc != TURBO_OK &&
        turbo_flow_gateway_coronet_server_snapshot(server, &snapshot) ==
            TURBO_OK)
      fprintf(stderr,
              "GATEWAY_SHUTDOWN_FAILURE transport=udp status=%d "
  ...
  it("keeps the event loop live until a pending TCP publication settles") {
    check_int_eq(gateway_test_tcp_settlement_shutdown_trace(), TURBO_OK);
  }
  ...
  it("enforces the OCPP path and subprotocol on WebSocket ingress") {
    ...
    turbo_flow_gateway_coronet_server_t *server = NULL;
    turbo_flow_gateway_coronet_config_t config;
    unsigned short port = gateway_test_pick_port(0);
    gateway_coronet_probe_init(&probe);
    memset(&client, 0, sizeof(client));
    check_true(port != 0u);
    check_int_eq(gateway_coronet_plugin_open(FLOW_GATEWAY_OCPP_MODULE, "ocpp",
                                             TURBO_FLOW_GATEWAY_PROTOCOL_OCPP, "2.0.1", &registry,
                                             &owner, &gateway),
                 TURBO_OK);
    gateway_coronet_config_init(&config, gateway, TURBO_FLOW_GATEWAY_CORONET_WS, port, &probe);
    config.path = "/ocpp";
    config.subprotocol = "ocpp2.0.1";
    check_int_eq(turbo_flow_gateway_coronet_server_create(&config, &server), TURBO_OK);
    check_int_eq(turbo_flow_gateway_coronet_server_start(server), TURBO_OK);
    client.port = port;
    client.payload = payload;
    check_int_eq(gateway_test_run_coronet_client(&client), TURBO_OK);
    check_true(client.done);
    check_int_eq(client.status, TURBO_OK);
    check_int_eq(gateway_test_wait_publishes(&probe, 1), TURBO_OK);
    check_int_eq(turbo_flow_gateway_coronet_server_begin_shutdown(server), TURBO_OK);
    check_int_eq(turbo_flow_gateway_coronet_server_wait_shutdown(server, GATEWAY_TEST_WAIT_MS),
                 TURBO_OK);
    check_int_eq(turbo_flow_gateway_coronet_server_destroy(server), TURBO_OK);
    gateway_coronet_plugin_close(registry, owner);
  }
  ...
  it("maps GB/T 32960 over an explicitly configured TLS listener") {
    ...
    check_int_eq(
        ...
        TURBO_OK);
    check_int_eq(gateway_coronet_plugin_open(FLOW_GATEWAY_GBT32960_MODULE, "gbt32960",
                                             TURBO_FLOW_GATEWAY_PROTOCOL_GBT_32960, "2025",
                                             &registry, &owner, &gateway),
                 TURBO_OK);
    gateway_coronet_config_init(&config, gateway, TURBO_FLOW_GATEWAY_CORONET_TLS, port, &probe);
    config.resolve_identity = NULL;
    config.identity_ctx = NULL;
    config.tls.cert_file = cert_file;
    config.tls.key_file = key_file;
  ...
  it("reconnects TLS clients without retaining session resources") {
    ...
    check_int_eq(turbo_flow_gateway_coronet_server_create(&config, &server), TURBO_OK);
    check_int_eq(turbo_flow_gateway_coronet_server_start(server), TURBO_OK);
    for (int iteration = 0; iteration < GATEWAY_TEST_RECONNECT_ITERATIONS; ++iteration) {
      gateway_ws_client_t client;
      turbo_flow_gateway_coronet_snapshot_t snapshot = TURBO_FLOW_GATEWAY_CORONET_SNAPSHOT_INIT;
      memset(&client, 0, sizeof(client));
      client.port = port;
      ...
      check_true(client.done);
      check_int_eq(client.status, TURBO_OK);
      check_int_eq(gateway_test_wait_publishes(&probe, iteration + 1), TURBO_OK);
      check_int_eq(gateway_test_wait_quiescent(server, &snapshot), TURBO_OK);
  ...
  it("releases a failed TLS startup before same-port recovery") {
    ...
    config.tls.key_file = key_file;
    check_int_eq(turbo_flow_gateway_coronet_server_create(&config, &server), TURBO_OK);
    check_int_eq(turbo_flow_gateway_coronet_server_start(server), TURBO_EIO);
    check_int_eq(turbo_flow_gateway_coronet_server_destroy(server), TURBO_OK);
    tls_test_remove_file(key_file);
    tls_test_remove_file(cert_file);
    check_int_eq(
  ...
  it("requires a verified client certificate before WSS identity admission") {
    check_int_eq(gateway_test_wss_mtls_identity_trace(GATEWAY_TEST_MTLS_NO_FAULT), TURBO_OK);
  }

  it("releases WSS plugin ownership when the identity trace aborts after open") {
    check_int_eq(gateway_test_wss_mtls_identity_trace(GATEWAY_TEST_MTLS_FAIL_AFTER_PLUGIN_OPEN),
                 TURBO_ECANCELED);
  }

  it("reconnects authenticated WSS clients without retaining session resources") {
    check_int_eq(gateway_test_wss_mtls_reconnect_trace(), TURBO_OK);
  }
}
