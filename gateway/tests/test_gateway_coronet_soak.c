...
#if defined(GATEWAY_SOAK_HAS_ASAN)
...
#ifndef GATEWAY_CORONET_SOAK_TEST
  #error "GATEWAY_CORONET_SOAK_TEST must point to test_gateway_coronet"
#endif

#define GATEWAY_SOAK_DEFAULT_ITERATIONS 5u
#define GATEWAY_SOAK_MAX_ITERATIONS 1000000u
#define GATEWAY_SOAK_CHILD_TIMEOUT_MS 120000u
#define GATEWAY_SOAK_PROCESS_RELEASE_TIMEOUT_MS 1000u
#define GATEWAY_SOAK_MAX_DURATION_MS (8u * 60u * 60u * 1000u)
#define GATEWAY_SOAK_LATENCY_SAMPLES 4096u
#define GATEWAY_SOAK_DEFAULT_RSS_TOLERANCE (8u * 1024u * 1024u)
#define GATEWAY_SOAK_MAX_RSS_TOLERANCE (1024u * 1024u * 1024u)
#define GATEWAY_SOAK_DEFAULT_SEED UINT64_C(0x4741544557415953)

typedef struct gateway_soak_resources_s {
  size_t rss_bytes;
  size_t handles;
  size_t threads;
} gateway_soak_resources_t;

typedef struct gateway_soak_trace_s {
  const char *name;
  const char *filter;
} gateway_soak_trace_t;

static const gateway_soak_trace_t gateway_soak_traces[] = {
...
static int gateway_soak_compare_u64(const void *left, const void *right) {
  const uint64_t a = *(const uint64_t *)left;
  const uint64_t b = *(const uint64_t *)right;
  return (a > b) - (a < b);
}

static uint64_t gateway_soak_percentile(uint64_t *values, size_t count, size_t percent) {
  size_t index;
  if (!values || count == 0u) return 0u;
  qsort(values, count, sizeof(*values), gateway_soak_compare_u64);
  index = ((count - 1u) * percent) / 100u;
  return values[index];
}

static uint64_t gateway_soak_random(uint64_t *state) {
...
static int gateway_soak_env_u64(const char *name, uint64_t fallback, uint64_t maximum,
...
static int gateway_soak_resource_snapshot(gateway_soak_resources_t *out) {
...
static int gateway_soak_wait_for_process_release(const gateway_soak_resources_t *baseline,
                                                 gateway_soak_resources_t *current) {
  const uint64_t deadline =
      turbo_monotonic_ms() + (uint64_t)GATEWAY_SOAK_PROCESS_RELEASE_TIMEOUT_MS;
  int rc;
  if (!baseline || !current) return TURBO_EINVAL;
  do {
    rc = gateway_soak_resource_snapshot(current);
...
static void gateway_soak_report_child_stream(turbo_process_t *process, int stdout_stream) {
  char buffer[4096];
  size_t count = 0u;
  int rc;
  if (!process) return;
...
static int gateway_soak_run_child(const gateway_soak_trace_t *trace, int verify_resource_release) {
  const char *args[3];
  turbo_process_options_t options;
  turbo_process_t *process = NULL;
  turbo_process_result_t result = {0};
  gateway_soak_resources_t resources_before;
  gateway_soak_resources_t resources_after;
  int release_rc;
  int rc;
  if (!trace || !trace->name || !trace->filter) return TURBO_EINVAL;
  rc = gateway_soak_resource_snapshot(&resources_before);
  if (rc != TURBO_OK) return rc;
  args[0] = "--filter";
  args[1] = trace->filter;
  args[2] = NULL;
  turbo_process_options_init(&options);
  options.program = GATEWAY_CORONET_SOAK_TEST;
  options.args = args;
  options.flags = TURBO_PROCESS_CAPTURE_STDOUT | TURBO_PROCESS_CAPTURE_STDERR;
  options.timeout_ms = GATEWAY_SOAK_CHILD_TIMEOUT_MS;
  ...
  if (rc == TURBO_OK && (result.state != TURBO_PROCESS_EXITED || result.exit_code != 0)) {
    ...
    gateway_soak_report_child_stream(process, 1);
    fputs("\nstderr:\n", stderr);
    gateway_soak_report_child_stream(process, 0);
    fputc('\n', stderr);
    rc = TURBO_EIO;
  } else if (rc != TURBO_OK) {
  ...
  release_rc = gateway_soak_wait_for_process_release(&resources_before, &resources_after);
  if (release_rc != TURBO_OK) {
    fprintf(stderr,
            ...
            GATEWAY_SOAK_PROCESS_RELEASE_TIMEOUT_MS);
    if (rc == TURBO_OK) rc = release_rc;
  }
  return rc;
}

static int gateway_soak_run_series(void) {
  uint64_t latencies[GATEWAY_SOAK_LATENCY_SAMPLES];
  size_t trace_counts[sizeof(gateway_soak_traces) / sizeof(gateway_soak_traces[0])] = {0};
  gateway_soak_resources_t baseline;
  gateway_soak_resources_t current;
  ...
  const size_t trace_count = sizeof(gateway_soak_traces) / sizeof(gateway_soak_traces[0]);
  const char *selected_trace_name = getenv("GATEWAY_TRANSPORT_SOAK_TRACE");
  size_t selected_trace = SIZE_MAX;
  uint64_t series_started_at;
  uint64_t deadline_ms;
  int rc;

  rc = gateway_soak_env_u64("GATEWAY_TRANSPORT_SOAK_ITERATIONS", GATEWAY_SOAK_DEFAULT_ITERATIONS,
                            GATEWAY_SOAK_MAX_ITERATIONS, 0, &iterations_value);
  if (rc == TURBO_OK)
    rc = gateway_soak_env_u64("GATEWAY_TRANSPORT_SOAK_DURATION_MS", 0u,
                              GATEWAY_SOAK_MAX_DURATION_MS, 1, &duration_ms);
  if (rc == TURBO_OK)
    rc = gateway_soak_env_u64("GATEWAY_TRANSPORT_SOAK_SEED", GATEWAY_SOAK_DEFAULT_SEED, UINT64_MAX,
                              0, &seed);
  if (rc == TURBO_OK)
    rc = gateway_soak_env_u64("GATEWAY_TRANSPORT_SOAK_RSS_TOLERANCE_BYTES",
                              GATEWAY_SOAK_DEFAULT_RSS_TOLERANCE, GATEWAY_SOAK_MAX_RSS_TOLERANCE, 1,
                              &rss_tolerance_value);
  if (rc != TURBO_OK) {
    fputs("SOAK_CONFIGURATION_FAILURE invalid gateway transport soak environment\n", stderr);
    return rc;
  }
  ...
  if (selected_trace_name && selected_trace_name[0]) {
    for (size_t trace_index = 0u; trace_index < trace_count; ++trace_index) {
      if (strcmp(selected_trace_name, gateway_soak_traces[trace_index].name) == 0) {
   ...
   */
  rc = gateway_soak_run_child(
      &gateway_soak_traces[selected_trace == SIZE_MAX ? 0u : selected_trace], 0);
  if (rc != TURBO_OK) return rc;
  rc = gateway_soak_resource_snapshot(&baseline);
  if (rc != TURBO_OK) return rc;
  current = baseline;
  ...
  while (completed < iterations || (deadline_ms != 0u && turbo_monotonic_ms() < deadline_ms)) {
    const size_t trace_index = selected_trace == SIZE_MAX
                                   ? (size_t)(gateway_soak_random(&random_state) % trace_count)
                                   : selected_trace;
    const uint64_t started_at = turbo_hrtime();
    uint64_t latency;
    rc = gateway_soak_run_child(&gateway_soak_traces[trace_index], 1);
    latency = turbo_hrtime() - started_at;
    ++completed;
    ++trace_counts[trace_index];
    if (latency_count < GATEWAY_SOAK_LATENCY_SAMPLES) {
      latencies[latency_count++] = latency;
    } else {
      const size_t slot = (size_t)(gateway_soak_random(&random_state) % completed);
      if (slot < GATEWAY_SOAK_LATENCY_SAMPLES) latencies[slot] = latency;
    }
    ...
    if (current.handles > baseline.handles || current.threads > baseline.threads ||
      ...
      fprintf(stderr,
              "SOAK_RESOURCE_FAILURE id=GATEWAY-SOAK-001 rss_baseline=%zu "
  ...
  printf("SOAK_RESULT id=GATEWAY-SOAK-001 seed=%" PRIu64
         ...
         gateway_soak_percentile(latencies, latency_count, 50u),
         gateway_soak_percentile(latencies, latency_count, 95u),
         gateway_soak_percentile(latencies, latency_count, 99u), baseline.rss_bytes,
         current.rss_bytes, baseline.handles, current.handles, baseline.threads, current.threads);
  return failures == 0u ? TURBO_OK : TURBO_EIO;
}

spec("Gateway CoroNet scheduled transport soak") {
  it("GATEWAY-SOAK-001 preserves transport lifecycle and parent resources") {
    check_int_eq(gateway_soak_run_series(), TURBO_OK);
  }
}
