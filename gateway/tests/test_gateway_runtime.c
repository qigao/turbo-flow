#include "tinytest.h"
#include "turbo_error.h"
#include "turbo_flow_gateway.h"
#include "turbo_flow_gateway_plugin.h"
#include "turbo_flow_gateway_runtime.h"

#include <stddef.h>
#include <string.h>

typedef struct runtime_probe_s {
  size_t publishes;
  size_t settlements;
  size_t closes;
  size_t replies;
  size_t pending_publish;
  uint64_t last_delivery_id;
  int last_settlement_status;
  int last_close_status;
  char last_topic[384];
  uint8_t last_reply[8];
  size_t last_reply_size;
} runtime_probe_t;

static int runtime_inspect(
    void *ctx, const char *configured_version,
    const turbo_flow_gateway_frame_view_t *frame,
    turbo_flow_gateway_metadata_t *metadata) {
  (void)ctx;
  (void)configured_version;
  if (!frame || !metadata || !frame->data || frame->data_size == 0u)
    return TURBO_EPROTO;
  metadata->message_type = frame->data[0];
  metadata->sequence = frame->data_size;
  memcpy(metadata->operation, "frame", sizeof("frame"));
  if (!frame->device_id)
    memcpy(metadata->device_id, "frame-device", sizeof("frame-device"));
  return TURBO_OK;
}

static int runtime_codec_reply(
    void *ctx, const char *configured_version,
    const turbo_flow_gateway_frame_view_t *request, int status,
    turbo_flow_gateway_frame_output_t *output) {
  (void)ctx;
  (void)configured_version;
  if (!request || !output || !output->data || output->capacity < 2u)
    return TURBO_EINVAL;
  output->data[0] = UINT8_C(0xac);
  output->data[1] = status == TURBO_OK ? 0u : 1u;
  output->data_size = 2u;
  return TURBO_OK;
}

static int runtime_publish(
    void *ctx, const turbo_flow_gateway_publish_request_t *request,
    turbo_flow_gateway_publish_disposition_t *disposition) {
  runtime_probe_t *probe = (runtime_probe_t *)ctx;
  if (!probe || !request || !request->message || !disposition)
    return TURBO_EINVAL;
  probe->publishes++;
  probe->last_delivery_id = request->delivery_id;
  if (request->message->topic_size >= sizeof(probe->last_topic))
    return TURBO_EMSGSIZE;
  memcpy(probe->last_topic, request->message->topic,
         request->message->topic_size + 1u);
  *disposition =
      probe->publishes == probe->pending_publish
          ? TURBO_FLOW_GATEWAY_PUBLISH_PENDING
          : TURBO_FLOW_GATEWAY_PUBLISH_SETTLED;
  return TURBO_OK;
}

static void runtime_settled(
    void *ctx, uint64_t session_id, uint64_t generation, uint64_t delivery_id,
    const turbo_flow_gateway_metadata_t *metadata, int status) {
  runtime_probe_t *probe = (runtime_probe_t *)ctx;
  (void)session_id;
  (void)generation;
  (void)delivery_id;
  (void)metadata;
  probe->settlements++;
  probe->last_settlement_status = status;
}

static void runtime_closed(void *ctx, uint64_t session_id,
                           uint64_t generation, int status) {
  runtime_probe_t *probe = (runtime_probe_t *)ctx;
  (void)session_id;
  (void)generation;
  probe->closes++;
  probe->last_close_status = status;
}

static int runtime_reply(
    void *ctx, uint64_t session_id, uint64_t generation,
    uint64_t delivery_id, const turbo_flow_gateway_frame_output_t *frame) {
  runtime_probe_t *probe = (runtime_probe_t *)ctx;
  (void)session_id;
  (void)generation;
  (void)delivery_id;
  if (!probe || !frame || frame->data_size > sizeof(probe->last_reply))
    return TURBO_EINVAL;
  memcpy(probe->last_reply, frame->data, frame->data_size);
  probe->last_reply_size = frame->data_size;
  probe->replies++;
  return TURBO_OK;
}

static int runtime_gateway_create(turbo_flow_gateway_protocol_t protocol,
                                  turbo_flow_gateway_t **out) {
  turbo_flow_gateway_open_request_t request =
      TURBO_FLOW_GATEWAY_OPEN_REQUEST_INIT;
  turbo_flow_gateway_codec_ops_t ops = TURBO_FLOW_GATEWAY_CODEC_OPS_INIT;
  request.protocol = protocol;
  request.protocol_version = "test";
  request.max_frame_size = 64u;
  ops.inspect = runtime_inspect;
  ops.reply = runtime_codec_reply;
  return turbo_flow_gateway_create(
      &request, "runtime-test", "test",
      TURBO_FLOW_GATEWAY_CAP_INGRESS | TURBO_FLOW_GATEWAY_CAP_EGRESS |
          TURBO_FLOW_GATEWAY_CAP_RAW_PRESERVE |
          TURBO_FLOW_GATEWAY_CAP_PROTOCOL_REPLY,
      &ops, NULL, out);
}

static int runtime_create(turbo_flow_gateway_t *gateway,
                          runtime_probe_t *probe, size_t max_sessions,
                          turbo_flow_gateway_runtime_t **out) {
  turbo_flow_gateway_runtime_config_t config =
      TURBO_FLOW_GATEWAY_RUNTIME_CONFIG_INIT;
  turbo_flow_gateway_runtime_ops_t ops =
      TURBO_FLOW_GATEWAY_RUNTIME_OPS_INIT;
  config.max_sessions = max_sessions;
  config.max_frame_size = 64u;
  config.max_buffered_bytes = (max_sessions + 1u) * config.max_frame_size;
  ops.publish = runtime_publish;
  ops.settled = runtime_settled;
  ops.session_closed = runtime_closed;
  ops.reply = runtime_reply;
  return turbo_flow_gateway_runtime_create(gateway, &config, &ops, probe,
                                           out);
}

static int runtime_session_open(turbo_flow_gateway_runtime_t *runtime,
                                uint64_t session_id,
                                const char *device_id) {
  turbo_flow_gateway_session_open_request_t request =
      TURBO_FLOW_GATEWAY_SESSION_OPEN_REQUEST_INIT;
  request.session_id = session_id;
  request.generation = 1u;
  request.device_id = device_id;
  request.protocol_version = "test";
  return turbo_flow_gateway_runtime_session_open(runtime, &request);
}

static void runtime_gbt_frame(uint8_t frame[25], uint8_t marker) {
  memset(frame, marker, 25u);
  frame[0] = 0x23u;
  frame[1] = 0x23u;
  frame[22] = 0u;
  frame[23] = 0u;
}

spec("gateway session runtime") {
  it("accepts the pre-reply runtime callback-table prefix") {
    runtime_probe_t probe = {0};
    turbo_flow_gateway_t *gateway = NULL;
    turbo_flow_gateway_runtime_t *runtime = NULL;
    turbo_flow_gateway_runtime_config_t config =
        TURBO_FLOW_GATEWAY_RUNTIME_CONFIG_INIT;
    turbo_flow_gateway_runtime_ops_t ops =
        TURBO_FLOW_GATEWAY_RUNTIME_OPS_INIT;
    check_int_eq(runtime_gateway_create(
                     TURBO_FLOW_GATEWAY_PROTOCOL_COAP, &gateway),
                 TURBO_OK);
    config.max_sessions = 1u;
    config.max_frame_size = 64u;
    config.max_buffered_bytes = 128u;
    ops.size = offsetof(turbo_flow_gateway_runtime_ops_t, reply);
    ops.publish = runtime_publish;
    check_int_eq(turbo_flow_gateway_runtime_create(
                     gateway, &config, &ops, &probe, &runtime),
                 TURBO_OK);
    check_int_eq(turbo_flow_gateway_runtime_begin_shutdown(runtime),
                 TURBO_OK);
    check_int_eq(turbo_flow_gateway_runtime_destroy(runtime), TURBO_OK);
    turbo_flow_gateway_destroy(gateway);
  }

  it("maps one message-boundary frame and settles it inline") {
    const uint8_t frame[] = {0x42u};
    runtime_probe_t probe = {0};
    turbo_flow_gateway_t *gateway = NULL;
    turbo_flow_gateway_runtime_t *runtime = NULL;
    turbo_flow_gateway_feed_result_t result =
        TURBO_FLOW_GATEWAY_FEED_RESULT_INIT;
    check_int_eq(runtime_gateway_create(TURBO_FLOW_GATEWAY_PROTOCOL_COAP,
                                        &gateway),
                 TURBO_OK);
    check_int_eq(runtime_create(gateway, &probe, 2u, &runtime), TURBO_OK);
    check_int_eq(runtime_session_open(runtime, 1u, "sensor-1"), TURBO_OK);
    check_int_eq(turbo_flow_gateway_runtime_session_feed(
                     runtime, 1u, 1u, frame, sizeof(frame), &result),
                 TURBO_OK);
    check_size_eq(result.accepted_size, sizeof(frame));
    check_size_eq(result.frames_dispatched, 1u);
    check_size_eq(probe.publishes, 1u);
    check_size_eq(probe.settlements, 1u);
    check_size_eq(probe.replies, 1u);
    check_size_eq(probe.last_reply_size, 2u);
    check_uint_eq(probe.last_reply[1], 0u);
    check_str_eq(probe.last_topic,
                 "gateway/coap/default/sensor-1/up/frame");
    check_int_eq(turbo_flow_gateway_runtime_begin_shutdown(runtime),
                 TURBO_OK);
    check_int_eq(turbo_flow_gateway_runtime_destroy(runtime), TURBO_OK);
    turbo_flow_gateway_destroy(gateway);
  }

  it("reassembles a fragmented GB/T 32960 stream frame") {
    uint8_t frame[25];
    runtime_probe_t probe = {0};
    turbo_flow_gateway_t *gateway = NULL;
    turbo_flow_gateway_runtime_t *runtime = NULL;
    turbo_flow_gateway_feed_result_t first =
        TURBO_FLOW_GATEWAY_FEED_RESULT_INIT;
    turbo_flow_gateway_feed_result_t second =
        TURBO_FLOW_GATEWAY_FEED_RESULT_INIT;
    runtime_gbt_frame(frame, 0x11u);
    check_int_eq(runtime_gateway_create(
                     TURBO_FLOW_GATEWAY_PROTOCOL_GBT_32960, &gateway),
                 TURBO_OK);
    check_int_eq(runtime_create(gateway, &probe, 1u, &runtime), TURBO_OK);
    check_int_eq(runtime_session_open(runtime, 2u, NULL), TURBO_OK);
    check_int_eq(turbo_flow_gateway_runtime_session_feed(
                     runtime, 2u, 1u, frame, 10u, &first),
                 TURBO_OK);
    check_size_eq(first.frames_dispatched, 0u);
    check_int_eq(turbo_flow_gateway_runtime_session_feed(
                     runtime, 2u, 1u, frame + 10u, sizeof(frame) - 10u,
                     &second),
                 TURBO_OK);
    check_size_eq(second.frames_dispatched, 1u);
    check_size_eq(probe.publishes, 1u);
    check_int_eq(turbo_flow_gateway_runtime_begin_shutdown(runtime),
                 TURBO_OK);
    check_int_eq(turbo_flow_gateway_runtime_destroy(runtime), TURBO_OK);
    turbo_flow_gateway_destroy(gateway);
  }

  it("buffers a following stream frame behind one pending settlement") {
    uint8_t frames[50];
    runtime_probe_t probe = {0};
    turbo_flow_gateway_t *gateway = NULL;
    turbo_flow_gateway_runtime_t *runtime = NULL;
    turbo_flow_gateway_feed_result_t feed =
        TURBO_FLOW_GATEWAY_FEED_RESULT_INIT;
    turbo_flow_gateway_feed_result_t settle =
        TURBO_FLOW_GATEWAY_FEED_RESULT_INIT;
    turbo_flow_gateway_runtime_snapshot_t snapshot =
        TURBO_FLOW_GATEWAY_RUNTIME_SNAPSHOT_INIT;
    runtime_gbt_frame(frames, 0x11u);
    runtime_gbt_frame(frames + 25u, 0x22u);
    probe.pending_publish = 1u;
    check_int_eq(runtime_gateway_create(
                     TURBO_FLOW_GATEWAY_PROTOCOL_GBT_32960, &gateway),
                 TURBO_OK);
    check_int_eq(runtime_create(gateway, &probe, 1u, &runtime), TURBO_OK);
    check_int_eq(runtime_session_open(runtime, 3u, NULL), TURBO_OK);
    check_int_eq(turbo_flow_gateway_runtime_session_feed(
                     runtime, 3u, 1u, frames, sizeof(frames), &feed),
                 TURBO_OK);
    check_size_eq(feed.frames_dispatched, 1u);
    check_true(feed.backpressured);
    check_uint_eq(feed.pending_delivery_id, 1u);
    check_size_eq(probe.replies, 0u);
    check_int_eq(turbo_flow_gateway_runtime_snapshot(runtime, &snapshot),
                 TURBO_OK);
    check_size_eq(snapshot.pending_settlements, 1u);
    check_size_eq(snapshot.buffered_bytes, sizeof(frames));
    check_int_eq(turbo_flow_gateway_runtime_settle(runtime, 1u, TURBO_OK,
                                                   &settle),
                 TURBO_OK);
    check_size_eq(settle.frames_dispatched, 1u);
    check_size_eq(probe.publishes, 2u);
    check_size_eq(probe.replies, 2u);
    snapshot = (turbo_flow_gateway_runtime_snapshot_t)
        TURBO_FLOW_GATEWAY_RUNTIME_SNAPSHOT_INIT;
    check_int_eq(turbo_flow_gateway_runtime_snapshot(runtime, &snapshot),
                 TURBO_OK);
    check_size_eq(snapshot.pending_settlements, 0u);
    check_size_eq(snapshot.buffered_bytes, 0u);
    check_int_eq(turbo_flow_gateway_runtime_begin_shutdown(runtime),
                 TURBO_OK);
    check_int_eq(turbo_flow_gateway_runtime_destroy(runtime), TURBO_OK);
    turbo_flow_gateway_destroy(gateway);
  }

  it("closes admission and waits only for admitted settlement ownership") {
    const uint8_t frame[] = {0x42u};
    runtime_probe_t probe = {0};
    turbo_flow_gateway_t *gateway = NULL;
    turbo_flow_gateway_runtime_t *runtime = NULL;
    turbo_flow_gateway_feed_result_t feed =
        TURBO_FLOW_GATEWAY_FEED_RESULT_INIT;
    turbo_flow_gateway_feed_result_t settle =
        TURBO_FLOW_GATEWAY_FEED_RESULT_INIT;
    turbo_flow_gateway_runtime_snapshot_t snapshot =
        TURBO_FLOW_GATEWAY_RUNTIME_SNAPSHOT_INIT;
    probe.pending_publish = 1u;
    check_int_eq(runtime_gateway_create(TURBO_FLOW_GATEWAY_PROTOCOL_COAP,
                                        &gateway),
                 TURBO_OK);
    check_int_eq(runtime_create(gateway, &probe, 1u, &runtime), TURBO_OK);
    check_int_eq(runtime_session_open(runtime, 4u, "sensor-4"), TURBO_OK);
    check_int_eq(turbo_flow_gateway_runtime_session_feed(
                     runtime, 4u, 1u, frame, sizeof(frame), &feed),
                 TURBO_OK);
    check_int_eq(turbo_flow_gateway_runtime_begin_shutdown(runtime),
                 TURBO_EBUSY);
    check_int_eq(runtime_session_open(runtime, 5u, "sensor-5"),
                 TURBO_ESHUTDOWN);
    check_int_eq(turbo_flow_gateway_runtime_settle(
                     runtime, feed.pending_delivery_id, TURBO_OK, &settle),
                 TURBO_OK);
    check_int_eq(turbo_flow_gateway_runtime_snapshot(runtime, &snapshot),
                 TURBO_OK);
    check_size_eq(snapshot.active_sessions, 0u);
    check_size_eq(snapshot.pending_settlements, 0u);
    check_false(snapshot.accepting);
    check_size_eq(probe.closes, 1u);
    check_int_eq(probe.last_close_status, TURBO_ESHUTDOWN);
    check_int_eq(turbo_flow_gateway_runtime_destroy(runtime), TURBO_OK);
    turbo_flow_gateway_destroy(gateway);
  }

  it("enforces the configured session and memory bounds") {
    runtime_probe_t probe = {0};
    turbo_flow_gateway_t *gateway = NULL;
    turbo_flow_gateway_runtime_t *runtime = NULL;
    turbo_flow_gateway_runtime_config_t invalid =
        TURBO_FLOW_GATEWAY_RUNTIME_CONFIG_INIT;
    turbo_flow_gateway_runtime_ops_t ops =
        TURBO_FLOW_GATEWAY_RUNTIME_OPS_INIT;
    ops.publish = runtime_publish;
    invalid.max_sessions = 2u;
    invalid.max_frame_size = 64u;
    invalid.max_buffered_bytes = 64u;
    check_int_eq(runtime_gateway_create(TURBO_FLOW_GATEWAY_PROTOCOL_COAP,
                                        &gateway),
                 TURBO_OK);
    check_int_eq(turbo_flow_gateway_runtime_create(
                     gateway, &invalid, &ops, &probe, &runtime),
                 TURBO_ENOSPC);
    check_null(runtime);
    check_int_eq(runtime_create(gateway, &probe, 1u, &runtime), TURBO_OK);
    check_int_eq(runtime_session_open(runtime, 6u, "sensor-6"), TURBO_OK);
    check_int_eq(runtime_session_open(runtime, 7u, "sensor-7"),
                 TURBO_ENOSPC);
    check_int_eq(turbo_flow_gateway_runtime_force_shutdown(
                     runtime, TURBO_ECANCELED),
                 TURBO_OK);
    check_int_eq(turbo_flow_gateway_runtime_destroy(runtime), TURBO_OK);
    turbo_flow_gateway_destroy(gateway);
  }
}
