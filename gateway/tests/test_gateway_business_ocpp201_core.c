...
static int ocpp201_core_business_open(turbo_flow_gateway_business_registry_t **out_registry,
                                      turbo_flow_gateway_business_owner_t **out_owner,
                                      turbo_flow_gateway_business_t **out_business) {
  turbo_flow_gateway_business_open_request_t request =
      TURBO_FLOW_GATEWAY_BUSINESS_OPEN_REQUEST_INIT;
  turbo_flow_gateway_business_registry_t *registry = NULL;
  turbo_flow_gateway_business_owner_t *owner = NULL;
  char reason[256];
  int rc;
  ...
  *out_business = NULL;
  ...
  rc = turbo_flow_gateway_business_registry_create(1u, &registry);
  if (rc != TURBO_OK) return rc;
  rc = turbo_flow_gateway_business_registry_load(
      registry, FLOW_GATEWAY_BUSINESS_OCPP201_CORE_MODULE, reason, sizeof(reason));
  if (rc == TURBO_OK)
    rc = turbo_flow_gateway_business_owner_create_registered(registry, OCPP201_CORE_BUSINESS,
                                                             &request, &owner);
  if (rc == TURBO_OK)
    rc = turbo_flow_gateway_business_owner_instance(owner, TURBO_FLOW_GATEWAY_PROTOCOL_OCPP,
                                                    out_business);
  if (rc != TURBO_OK) {
    turbo_flow_gateway_business_owner_destroy(owner);
    (void)turbo_flow_gateway_business_registry_destroy(registry);
    return rc;
  }
...
static void ocpp201_core_business_close(turbo_flow_gateway_business_registry_t *registry,
                                        turbo_flow_gateway_business_owner_t *owner) {
  turbo_flow_gateway_business_owner_destroy(owner);
  check_int_eq(turbo_flow_gateway_business_registry_destroy(registry), TURBO_OK);
}

static turbo_flow_gateway_business_event_view_t
ocpp201_core_event(uint64_t delivery_id, const uint8_t *payload, size_t payload_size,
  ...
  turbo_flow_gateway_business_event_view_t event = TURBO_FLOW_GATEWAY_BUSINESS_EVENT_VIEW_INIT;

  event.delivery_id = delivery_id;
  event.session_id = 7u;
  event.session_generation = 3u;
  event.topic = "gateway/ocpp/fleet-a/charger-1/up/call";
  event.topic_size = strlen(event.topic);
  event.metadata.protocol = TURBO_FLOW_GATEWAY_PROTOCOL_OCPP;
  event.metadata.direction = TURBO_FLOW_GATEWAY_DIRECTION_UP;
...
static int ocpp201_core_gateway_open(turbo_flow_gateway_registry_t **out_registry,
                                     turbo_flow_gateway_owner_t **out_owner,
                                     turbo_flow_gateway_t **out_gateway) {
  ...
  if (!out_registry || !out_owner || !out_gateway) return TURBO_EINVAL;
  *out_registry = NULL;
  *out_owner = NULL;
  *out_gateway = NULL;
  request.protocol = TURBO_FLOW_GATEWAY_PROTOCOL_OCPP;
  ...
  if (rc != TURBO_OK) return rc;
  rc = turbo_flow_gateway_registry_load(registry, FLOW_GATEWAY_OCPP_MODULE, reason, sizeof(reason));
  if (rc == TURBO_OK)
    rc = turbo_flow_gateway_owner_create_registered(registry, "ocpp", &request, &owner);
  if (rc == TURBO_OK)
    rc = turbo_flow_gateway_owner_instance(owner, TURBO_FLOW_GATEWAY_PROTOCOL_OCPP, out_gateway);
  if (rc != TURBO_OK) {
    turbo_flow_gateway_owner_destroy(owner);
    (void)turbo_flow_gateway_registry_destroy(registry);
    return rc;
  }
  *out_registry = registry;
  *out_owner = owner;
...
spec("OCPP 2.0.1 Core DataBind business provider") {
  it("rejects an unsupported business profile before opening") {
    ...
    request.max_payload_size = 4096u;
    check_int_eq(turbo_flow_gateway_business_registry_create(1u, &registry), TURBO_OK);
    check_int_eq(turbo_flow_gateway_business_registry_load(
                     registry, FLOW_GATEWAY_BUSINESS_OCPP201_CORE_MODULE, reason, sizeof(reason)),
                 TURBO_OK);
    check_int_eq(turbo_flow_gateway_business_owner_create_registered(
  ...
  it("accepts a committed typed BootNotification body") {
    ...
    turbo_flow_gateway_business_registry_t *registry = NULL;
    turbo_flow_gateway_business_owner_t *owner = NULL;
    turbo_flow_gateway_business_t *business = NULL;
    turbo_flow_gateway_business_event_view_t event =
    ...
    check_int_eq(turbo_flow_gateway_business_consume_committed(business, &event), TURBO_OK);
  ...
  it("rejects a BootNotification missing a required nested field") {
    ...
    check_int_eq(ocpp201_core_business_open(&registry, &owner, &business), TURBO_OK);
    check_int_eq(turbo_flow_gateway_business_consume_committed(business, &event), TURBO_EPROTO);
    ocpp201_core_business_close(registry, owner);
  }
  ...
  it("rejects envelope metadata disagreement and unsupported Core actions") {
    ...
    check_int_eq(turbo_flow_gateway_business_consume_committed(business, &event), TURBO_ENOTSUP);
    ocpp201_core_business_close(registry, owner);
  }
  ...
  it("maps a typed reset command through the public OCPP encoder") {
    ...
    uint8_t frame[256];
    turbo_flow_gateway_business_registry_t *business_registry = NULL;
    turbo_flow_gateway_business_owner_t *business_owner = NULL;
    turbo_flow_gateway_business_t *business = NULL;
    turbo_flow_gateway_registry_t *gateway_registry = NULL;
    turbo_flow_gateway_owner_t *gateway_owner = NULL;
    turbo_flow_gateway_t *gateway = NULL;
    turbo_flow_gateway_business_command_request_t request =
        TURBO_FLOW_GATEWAY_BUSINESS_COMMAND_REQUEST_INIT;
    turbo_flow_gateway_business_command_output_t output =
        TURBO_FLOW_GATEWAY_BUSINESS_COMMAND_OUTPUT_INIT;
    turbo_flow_gateway_command_view_t command = TURBO_FLOW_GATEWAY_COMMAND_VIEW_INIT;
    turbo_flow_gateway_frame_output_t encoded = TURBO_FLOW_GATEWAY_FRAME_OUTPUT_INIT;

    check_int_eq(ocpp201_core_business_open(&business_registry, &business_owner, &business),
                 TURBO_OK);
    check_int_eq(ocpp201_core_gateway_open(&gateway_registry, &gateway_owner, &gateway), TURBO_OK);
    ...
    check_int_eq(turbo_flow_gateway_business_prepare_command(business, &request, &output),
                 TURBO_OK);
    ...
    check_int_eq(turbo_flow_gateway_business_command_view(&output, &command), TURBO_OK);
    encoded.data = frame;
    encoded.capacity = sizeof(frame);
    check_int_eq(turbo_flow_gateway_encode(gateway, &command, &encoded), TURBO_OK);
    frame[encoded.data_size] = '\0';
    check_str_eq((const char *)frame, "[2,\"cmd-11\",\"Reset\",{\"type\":\"Immediate\"}]");

    turbo_flow_gateway_owner_destroy(gateway_owner);
    check_int_eq(turbo_flow_gateway_registry_destroy(gateway_registry), TURBO_OK);
