...
typedef struct business_probe_s {
  ...
  char last_action[TURBO_FLOW_GATEWAY_OPERATION_MAX + 1u];
} business_probe_t;
...
static int business_probe_open(void *ctx, const turbo_flow_gateway_business_open_request_t *request,
  ...
  rc = turbo_flow_gateway_business_create("probe-biz", request->protocol, request->profile,
...
static void business_probe_close(void *ctx, turbo_flow_gateway_business_service_t *service) {
  business_probe_t *probe = (business_probe_t *)ctx;
  turbo_flow_gateway_business_destroy((turbo_flow_gateway_business_t *)service->owner);
  service->instance = NULL;
  service->owner = NULL;
  probe->closes++;
}

static turbo_flow_gateway_business_plugin_api_t business_probe_api(business_probe_t *probe) {
  turbo_flow_gateway_business_plugin_api_t api = {
...
spec("gateway business service") {
  it("keeps the plugin export limited while an opaque owner is alive") {
    business_probe_t probe = {0};
    turbo_flow_gateway_business_plugin_api_t api = business_probe_api(&probe);
    ...
    turbo_flow_gateway_business_t *business = NULL;
    turbo_flow_gateway_business_info_t info = TURBO_FLOW_GATEWAY_BUSINESS_INFO_INIT;
    ...
    check_int_eq(turbo_flow_gateway_business_registry_register(registry, &api), TURBO_OK);
    check_int_eq(turbo_flow_gateway_business_registry_register(registry, &api), TURBO_EALREADY);
    check_int_eq(turbo_flow_gateway_business_owner_create_registered(registry, "probe-biz",
                                                                     &request, &owner),
                 TURBO_OK);
    check_int_eq(turbo_flow_gateway_business_owner_instance(owner, TURBO_FLOW_GATEWAY_PROTOCOL_OCPP,
                                                            &business),
                 TURBO_OK);
    check_int_eq(turbo_flow_gateway_business_get_info(business, &info), TURBO_OK);
    check_str_eq(info.business, "probe-biz");
    check_str_eq(info.profile, "ocpp-2.0.1-core");
    check_int_eq(turbo_flow_gateway_business_registry_destroy(registry), TURBO_EBUSY);
  ...
  it("delivers only a borrowed post-commit schema identity") {
    ...
    ops.consume_committed = business_probe_consume;
    check_int_eq(turbo_flow_gateway_business_create(
                     "event-probe", TURBO_FLOW_GATEWAY_PROTOCOL_OCPP, "ocpp-2.0.1-core", 128u,
                     TURBO_FLOW_GATEWAY_BUSINESS_CAP_COMMITTED_EVENT, &ops, &probe, &business),
                 TURBO_OK);
    event.delivery_id = 41u;
    event.session_id = 7u;
    event.session_generation = 2u;
    event.topic = "gateway/ocpp/default/charger-1/up/Heartbeat";
    ...
    check_int_eq(turbo_flow_gateway_business_consume_committed(business, &event), TURBO_EINVAL);
    check_size_eq(probe.events, 1u);
    event.content.type_name = "HeartbeatRequest";
    probe.event_status = TURBO_EBUSY;
    check_int_eq(turbo_flow_gateway_business_consume_committed(business, &event), TURBO_EBUSY);
    check_size_eq(probe.events, 2u);
    turbo_flow_gateway_business_destroy(business);
  }
  ...
  it("maps a schema-bound business action into caller-owned command data") {
    ...
    ops.prepare_command = business_probe_prepare;
    check_int_eq(turbo_flow_gateway_business_create(
                     "command-probe", TURBO_FLOW_GATEWAY_PROTOCOL_JTT_808, "jtt808-2019", 64u,
                     TURBO_FLOW_GATEWAY_BUSINESS_CAP_PREPARE_COMMAND, &ops, &probe, &business),
                 TURBO_OK);
