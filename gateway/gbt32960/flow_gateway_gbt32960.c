...
static int flow_gbt32960_inspect(
...
static int flow_gbt32960_frame_write(
    uint8_t command, uint8_t response, const char *device_id,
...
static int flow_gbt32960_reply(
  ...
  return flow_gbt32960_frame_write(
      request->data[2], flow_gbt32960_response(status), device_id, NULL, 0u,
      output);
}
...
static int flow_gbt32960_command(const char *operation, uint8_t *out) {
  ...
  if (strcmp(operation, "vehicle-login") == 0)
    ...
    *out = UINT8_C(0x02);
  else if (strcmp(operation, "reissue-data") == 0)
    *out = UINT8_C(0x03);
  else if (strcmp(operation, "vehicle-logout") == 0)
    *out = UINT8_C(0x04);
  else if (strcmp(operation, "platform-login") == 0)
    *out = UINT8_C(0x05);
  else if (strcmp(operation, "platform-logout") == 0)
    *out = UINT8_C(0x06);
...
static const flow_gateway_plugin_descriptor_t FLOW_GBT32960_DESCRIPTOR = {
    "gbt32960", TURBO_FLOW_GATEWAY_PROTOCOL_GBT_32960, "2025",
    FLOW_GBT32960_VERSIONS,
    sizeof(FLOW_GBT32960_VERSIONS) / sizeof(FLOW_GBT32960_VERSIONS[0]),
    flow_gbt32960_inspect, flow_gbt32960_reply, flow_gbt32960_encode, NULL};

static const turbo_flow_gateway_plugin_api_t FLOW_GBT32960_API = {
