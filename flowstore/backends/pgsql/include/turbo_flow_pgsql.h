 ...
 *
 * Sink acceptance is reported only after COMMIT. Payload and message type/flags
 * are committed together. A source holds a PostgreSQL session advisory lock
 * while synchronously publishing one owned message and deletes the row only
 * after graph success. Connection loss releases the lock; a crash after graph
 * side effects and before DELETE can redeliver the row.
 */
