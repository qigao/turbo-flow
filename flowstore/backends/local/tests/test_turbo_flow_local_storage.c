...
spec("turbo_flow_local_storage_backend") {
  it("opens an in-memory record service through the common owner ABI") {
    ...
    check_equal(turbo_flow_storage_backend_owner_create_registered(registry, "local", &request,
                                                                    &owner, &error),
                 TURBO_OK);
    ...
    check_equal(store->scan(store->ctx, local_record_count, &count), TURBO_OK);
  ...
  it("RECORD-STORE-ENDURANCE-001 runs the shared revision trace through local memory") {
    turbo_flow_storage_backend_registry_t *registry = NULL;
    turbo_flow_storage_backend_owner_t *owner = NULL;
    turbo_flow_storage_backend_open_request_t request =
        TURBO_FLOW_STORAGE_BACKEND_OPEN_REQUEST_INIT;
    ...
    options.max_key_size = TURBO_FLOW_TEST_RECORD_ENDURANCE_KEY_SIZE;
    options.max_value_size = TURBO_FLOW_TEST_RECORD_ENDURANCE_VALUE_SIZE;
    options.max_batch_size = TURBO_FLOW_TEST_RECORD_ENDURANCE_BATCH_SIZE;
    options.max_records = TURBO_FLOW_TEST_RECORD_ENDURANCE_RECORDS;
    check_equal(turbo_flow_storage_backend_registry_create(1u, &registry), TURBO_OK);
    check_equal(turbo_flow_storage_backend_registry_register(
                     registry, turbo_flow_local_storage_backend_api()),
                 TURBO_OK);
