...
The two storage boundaries are intentionally different: successful XADD means
Redis accepted the append, while XACK after successful graph publication means
the consumer-group delivery completed. XADD success must never be reported as
downstream graph completion. If command outcome is uncertain, the error is
propagated and the caller retains responsibility for retry policy.
...
For asynchronous worker replies,
...
borrowed `turbo_flow_claim_settler_t` for an external coordinator. ACK and
drop both use exact XACK storage settlement. An uncertain XACK result keeps the
...
memory fallback. The maximum key size is 65538 bytes and supports bounded binary
application keys.
...
redis.key = "turboflow:{cluster-a:owner:42}:state";
