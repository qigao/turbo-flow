...
spec("storage backend registry") {
  it("selects by backend and capability while retaining active owners") {
    ...
    check_equal(turbo_flow_storage_backend_registry_create(2u, &registry), TURBO_OK);
    check_equal(turbo_flow_storage_backend_registry_register(registry, &api), TURBO_OK);
    check_equal(turbo_flow_storage_backend_registry_register(registry, &api), TURBO_EALREADY);
    check_equal((const void *)turbo_flow_storage_backend_registry_find(registry, "test"),
                (const void *)&api);
    check_equal(turbo_flow_storage_backend_owner_create_registered(
                     registry, "test", &request, &owner, NULL),
                 TURBO_OK);
    check_equal(turbo_flow_storage_backend_owner_service(
                     owner, TURBO_FLOW_STORAGE_MODEL_RECORD, &service),
                 TURBO_OK);
    check_equal(*(const int *)service, 42);
    check_equal(turbo_flow_storage_backend_owner_backend(owner), "test");
    check_equal(turbo_flow_storage_backend_registry_destroy(registry), TURBO_EBUSY);
    turbo_flow_storage_backend_owner_destroy(owner);
    check_equal(context.opens, 1u);
    check_equal(context.closes, 1u);
  ...
  it("enforces ABI shape and bounded registration") {
    ...
    check_equal(turbo_flow_storage_backend_registry_create(1u, &registry), TURBO_OK);
    check_equal(turbo_flow_storage_backend_registry_register(registry, &invalid), TURBO_EINVAL);
    check_equal(turbo_flow_storage_backend_registry_register(registry, &api), TURBO_OK);
    check_equal(turbo_flow_storage_backend_registry_register(registry, &second), TURBO_ENOSPC);
    check_equal(turbo_flow_storage_backend_registry_destroy(registry), TURBO_OK);
  }
}
