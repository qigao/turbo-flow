...
static int flow_pgsql_outbox_sink_commit(flow_pgsql_outbox_adapter_t *adapter,
  ...
  static const char insert_row[] =
      ...
      "origin_protocol_version,origin_session_id) "
      "VALUES($1,$2::bytea,$3::bigint,$4::bigint,$5::integer,$6::bigint,$7::numeric)";
  const char *one_value[1] = {adapter->outbox_name};
  const char *insert_values[7] = {0};
  int insert_lengths[7] = {0};
  int insert_formats[7] = {0};
  ...
  const turbo_flow_protocol_origin_t *origin = turbo_flow_msg_protocol_origin(msg);
  turbo_flow_protocol_origin_t derived_origin = TURBO_FLOW_PROTOCOL_ORIGIN_INIT;
  char message_type[32];
  char message_flags[32];
  char origin_protocol[32];
  char origin_protocol_version[32];
  char origin_session_id[32];
  PGresult *result = NULL;
  size_t depth = 0u;
  int rc;
  int written;
  if (!origin && route && settlement) {
    derived_origin.protocol = route->protocol;
    derived_origin.protocol_version = settlement->message.protocol_version;
    derived_origin.session_id = route->session_id;
    if (turbo_flow_protocol_origin_validate(&derived_origin) != TURBO_OK) return TURBO_EPROTO;
    origin = &derived_origin;
  }
  if (origin && route &&
      (origin->protocol != route->protocol || origin->session_id != route->session_id))
    return TURBO_EPROTO;
  if (origin && settlement &&
      (origin->protocol != settlement->message.protocol ||
       origin->protocol_version != settlement->message.protocol_version))
    return TURBO_EPROTO;
  ...
  if (origin) {
    written = snprintf(origin_protocol, sizeof(origin_protocol), "%u", (unsigned)origin->protocol);
    if (written < 0 || (size_t)written >= sizeof(origin_protocol)) return TURBO_ERANGE;
    written = snprintf(origin_protocol_version, sizeof(origin_protocol_version), "%u",
                       origin->protocol_version);
    if (written < 0 || (size_t)written >= sizeof(origin_protocol_version)) return TURBO_ERANGE;
    written = snprintf(origin_session_id, sizeof(origin_session_id), "%llu",
                       (unsigned long long)origin->session_id);
    if (written < 0 || (size_t)written >= sizeof(origin_session_id)) return TURBO_ERANGE;
  }
  ...
  insert_values[4] = origin ? origin_protocol : NULL;
  insert_values[5] = origin ? origin_protocol_version : NULL;
  insert_values[6] = origin ? origin_session_id : NULL;
  insert_lengths[1] = (int)msg->payload.len;
  insert_formats[1] = 1;
  result = PQexecParams(adapter->connection, insert_row, 7, NULL, insert_values, insert_lengths,
                        insert_formats, 0);
