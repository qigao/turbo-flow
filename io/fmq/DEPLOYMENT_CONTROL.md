# FMQ deployment control and rolling upgrade contract
...
1.0；若新 release 只能写 1.1，而旧 release 不能读 1.1，必须停机迁移。FMQ v3 若未来出现，只有
...
FMQ、Queue、Redis contract 和真实 Redis tests 共享 CTest label `fmq-release`；故障注入、restart、lease、
slow-peer 与 storage recovery 子集另有 `fmq-chaos`。发布构建必须显式打开真实 Redis，不能把 Disabled
视为通过：

```powershell
cmake --preset win-release-user -DTURBO_FLOW_REDIS_LIVE_TESTS=ON
cmake --build --preset win-release-user
ctest --test-dir build/Msvc-Release -C Release -L fmq-release --output-on-failure
```

Linux 使用对应 release preset。CI 应保存 TinyTest benchmark 输出和 CTest duration，按目标机器维护
P50/P95/P99、throughput、总耗时与失败率趋势；仓库中的数值只作为同机回归基线，不声明跨机器 SLA。
