...
现有 Chapter 3/4 broker 把 `READY -> dispatch -> completion` 表达为一个隐式 message credit：
...
目标是增加一个配置驱动的高级 pattern，而不是增加 FMQ socket pattern：
...
- SQLite Queue 使用独立 durable row/token，schema version 2 通过 queue 私有 metadata 表管理；legacy
  messages 表启动时原位迁移，未来 schema version fail fast；
...
- TFCW encode/decode 与 thin ROUTER/DEALER graph stage。
...
安全仍沿用当前阶段决策：协议只在宿主建立的可信 transport/network boundary 内使用，不新增认证、
