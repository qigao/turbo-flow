...
CXX_C_API int turbo_flow_fmq_register_fanout_adapter(
...
CXX_C_API int turbo_flow_fmq_register_fanout_adapter_ex(
    turbo_flow_t *flow, const char *name, const turbo_flow_fmq_config_t *config,
    const turbo_flow_fmq_fanout_config_t *fanout,
    const turbo_flow_coronet_execution_binding_t *execution);
...
/** Borrowed topic metadata, valid only during the current synchronous dispatch. */
CXX_C_API int turbo_flow_fmq_message_topic(const turbo_flow_msg_t *msg, tstr_v *topic);

/** Borrowed peer identity, valid only during the current synchronous dispatch. */
CXX_C_API int turbo_flow_fmq_message_identity(const turbo_flow_msg_t *msg, tstr_v *identity);

/** Borrowed wire correlation ID, valid only during the current synchronous dispatch. */
 ...
 *
 * The function copies the live route and content descriptor into message-owned
 * storage, then clears the borrowed `transport_context`. The message may then
 * cross worker/fan-out/memory-queue boundaries and later enter the same ROUTER
 * sink as a delayed reply. A disconnected, reconnected, or restarted session
 * fails with TURBO_ENOTCONN. REP remains deliberately synchronous.
 */
CXX_C_API int turbo_flow_fmq_message_detach_router_route(turbo_flow_msg_t *msg);

/**
 * Read a borrowed XPUB subscription event during synchronous dispatch.
