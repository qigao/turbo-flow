...
spec("flow_fmq_config") {
  ...
  it("registers bounded per-peer fan-out only from a complete YAML policy") {
    ...
    static const char wrong_policy[] =
        "version: 1\n"
        "adapters:\n"
        "  fmq.out:\n"
        "    kind: fmq\n"
  ...
  it("runs two connected FMQ adapters on one stable CoroNet pool lane") {
