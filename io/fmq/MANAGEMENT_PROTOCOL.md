...
未知 optional 字段被跳过；未知 critical 字段或 command type 返回 `TURBO_ENOTSUP`，由未来
management adapter 映射为稳定的 `UNSUPPORTED_CAPABILITY` status。schema validator 不持有
...
- 当前 owner 通过普通 CoroNet/FMQ REP graph 的薄 stage 接收请求；operation execution 仍由宿主在
  同一 owner lane 显式调用 `run_one()`，尚未另建 actor mailbox。后续接入 actor 时必须保留同一
  dedup 事实源，不能在 REP stage 和 actor 各维护一份状态。
...
会在启动时失败。
`io/fmq/examples/fmq.yml` 给出 SQLite 装配配置。
...
| TurboUtils `SoaParser` | 固定宽度批量列式数据高效 | 依赖全局 schema registry、little-endian、缺少稀疏异构管理字段演进 | 不用于 RPC；未来可作为 metrics/event payload schema |
