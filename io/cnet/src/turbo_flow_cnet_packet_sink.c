...
static void packet_sink_fail(turbo_flow_cnet_packet_sink_t *sink, int status) {
  ...
  if (state == TURBO_FLOW_CNET_PACKET_SINK_STOPPING ||
      state == TURBO_FLOW_CNET_PACKET_SINK_STOPPED ||
      state == TURBO_FLOW_CNET_PACKET_SINK_DETACHED)
    return;
...
static packet_sink_operation_t *
packet_sink_operation_from_tag(turbo_flow_cnet_packet_sink_t *sink, uint64_t tag) {
  ...
  if (!atomic_load_explicit(&operation->occupied, memory_order_acquire) ||
      !atomic_load_explicit(&operation->ready, memory_order_acquire) ||
      operation->tag != tag ||
...
static void packet_sink_actor_completion(void *user, cflow_io_request_id request_id,
  ...
  if (expected_request_id == 0u) {
    (void)atomic_compare_exchange_strong_explicit(
        &operation->request_id, &expected_request_id, request_id, memory_order_acq_rel,
        memory_order_acquire);
    expected_request_id = atomic_load_explicit(&operation->request_id, memory_order_acquire);
  }
...
static void packet_sink_on_send_terminal(void *user, cnet_packet_endpoint *endpoint,
  ...
  completed = cflow_io_actor_complete(
      &sink->actor, atomic_load_explicit(&operation->request_id, memory_order_acquire), &completion);
...
static int packet_sink_start(void *ctx, turbo_flow_t *flow,
  ...
  if (status != SALTS_OK) goto fail;
  status = cnet_packet_session_open(&sink->endpoint, &sink->peer, sink->conversation,
                                    &sink->session);
  if (status != SALTS_OK) goto fail;
  salts_mutex_lock(&sink->lifecycle_mutex);
...
static int packet_sink_stop_internal(turbo_flow_cnet_packet_sink_t *sink) {
  ...
  while (sink->owner_active) salts_cond_wait(&sink->lifecycle_cond, &sink->lifecycle_mutex);
  if (packet_sink_state(sink) == TURBO_FLOW_CNET_PACKET_SINK_STOPPED ||
      packet_sink_state(sink) == TURBO_FLOW_CNET_PACKET_SINK_DETACHED) {
    salts_mutex_unlock(&sink->lifecycle_mutex);
    return SALTS_OK;
...
#endif
  while (endpoint_drained && sink->actor_initialized &&
         !cflow_io_actor_is_quiescent(&sink->actor))
    packet_sink_drive_actor(sink);
  ...
  if (status == SALTS_OK && !sink->actor_initialized && !sink->executor_initialized &&
    ...
    sink->bound_port = 0u;
    atomic_store_explicit(&sink->session_open, false, memory_order_release);
    atomic_store_explicit(&sink->state, TURBO_FLOW_CNET_PACKET_SINK_STOPPED,
...
static void packet_sink_stop(void *ctx, turbo_flow_t *flow,
                             const turbo_flow_stage_plan_t *stage) {
  int status;
  (void)stage;
...
int turbo_flow_cnet_packet_sink_register(const turbo_flow_cnet_packet_sink_config_t *config,
  ...
  *sink_out = NULL;
  ...
  sink->operations = (packet_sink_operation_t *)calloc(sink->send_capacity,
                                                        sizeof(*sink->operations));
  sink->delivered_ids =
...
static void packet_sink_snapshot_locked(
    const turbo_flow_cnet_packet_sink_t *sink,
...
int turbo_flow_cnet_packet_sink_snapshot(const turbo_flow_cnet_packet_sink_t *sink,
  ...
  packet_sink_snapshot_locked(sink, snapshot);
  salts_mutex_unlock(&mutable_sink->lifecycle_mutex);
  return SALTS_OK;
}
