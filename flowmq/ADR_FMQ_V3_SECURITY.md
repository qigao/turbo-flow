...
测试与部署文档，因此需要明确记录协议、状态所有权、迁移和回滚约束。
...
3. 配置 security binding 后，安全要求是强制且不可降级的：
   - TCP/TLS/UDP/KCP/Pipe/WS/WSS 都可执行相同 authentication 和 default-deny ACL；
   - TLS/WSS 必须经过 client peer verification，并强制 RFC 9266 TLS exporter channel binding；
   - TCP/UDP/KCP/Pipe/WS 不提供 credential confidentiality，生产环境必须由可信网络或额外安全隧道保护；
...
- TLS/WSS 具备 exporter channel binding；TCP/UDP/KCP/Pipe/WS 只提供应用层认证和 ACL，不提供链路保密。
