...
static void fmq_bench_tcp_one_way(const fmq_bench_throughput_case_t *bench_case) {
  ...
  check_true(!bench_case->use_batch_api ||
  ...
  if (bench_case->warmup_messages > 1u) {
    ...
    rc = bench_case->use_batch_api
             ? fmq_bench_send_api_batch(sender, &receive_state, batch_items, remaining_warmup,
                                        expected_received, NULL)
             : fmq_bench_send_batch(sender, &receive_state, payload, bench_case->payload_bytes,
  ...
  benchmark_io(bench_case->title, bench_case->samples, bench_case->messages_per_sample,
    ...
    if (rc == TURBO_OK) {
      ...
      rc = bench_case->use_batch_api
               ? fmq_bench_send_api_batch(sender, &receive_state, batch_items,
                                          bench_case->messages_per_sample, expected_received,
                                          &latencies[sample_index])
               : fmq_bench_send_batch(sender, &receive_state, payload,
