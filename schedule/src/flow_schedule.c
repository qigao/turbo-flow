...
struct turbo_flow_schedule_s {
  ...
  turbo_mutex_t mutex;
  turbo_timer_t *timer;
  turbo_thread_t bootstrap_thread;
...
static void flow_schedule_publish_complete(void *ctx, const turbo_flow_publish_result_t *result) {
  ...
  int status = result ? result->status : TURBO_EINVAL;
    ...
    if (schedule->mode != TURBO_FLOW_SCHEDULE_CRON) {
      ...
      if (schedule->mode == TURBO_FLOW_SCHEDULE_ONE_SHOT ||
        ...
        atomic_store_explicit(&schedule->next_due_ms,
                              flow_schedule_add_delay(turbo_monotonic_ms(), schedule->delay_ms),
                              memory_order_release);
      }
    }
    if (!atomic_load_explicit(&schedule->completed, memory_order_acquire)) {
      atomic_store_explicit(&schedule->last_status, TURBO_OK, memory_order_release);
    }
  } else {
...
static int flow_schedule_advance_cron(turbo_flow_schedule_t *schedule, time_t now) {
  ...
  int truncated = 0;
  int rc;
  if (!schedule || schedule->mode != TURBO_FLOW_SCHEDULE_CRON) return TURBO_EINVAL;
  current_minute = flow_schedule_floor_minute(now);
  turbo_mutex_lock(&schedule->mutex);
  if (schedule->cron_cursor_minute != 0 && current_minute <= schedule->cron_cursor_minute) {
    turbo_mutex_unlock(&schedule->mutex);
    return 0;
  }
  ...
  for (;;) {
    rc = turbo_cron_next(&schedule->cron_expr, check, &next_fire);
    if (rc != TURBO_CRON_OK) {
      atomic_store_explicit(&schedule->last_status, TURBO_EINVAL, memory_order_release);
      return TURBO_EINVAL;
    }
...
static void flow_schedule_timer_callback(turbo_timer_t *timer) {
  turbo_flow_schedule_t *schedule = (turbo_flow_schedule_t *)turbo_timer_get_data(timer);
  int stop_timer = 0;
  int rc = TURBO_OK;
  if (!schedule ||
  ...
  if (stop_timer) (void)turbo_timer_stop(timer);
...
static int flow_schedule_arm_timer(turbo_flow_schedule_t *schedule) {
  ...
  if (schedule->mode == TURBO_FLOW_SCHEDULE_CRON) {
    ...
    if (turbo_cron_next(&schedule->cron_expr, now, &next_fire) != TURBO_CRON_OK ||
        next_fire <= now) {
      return TURBO_EINVAL;
    }
  ...
  return turbo_timer_start(schedule->timer, flow_schedule_timer_callback, timeout_ms, repeat_ms) ==
                 0
             ? TURBO_OK
             : TURBO_EIO;
}
...
static void flow_schedule_bootstrap_thread(void *ctx) {
  ...
  while (atomic_load_explicit(&schedule->started, memory_order_acquire) && schedule->flow &&
    ...
    turbo_sleep_ms(FLOW_SCHEDULE_START_POLL_MS);
  }
...
static int flow_schedule_start(void *ctx, turbo_flow_t *flow,
  ...
  turbo_flow_schedule_t *schedule = (turbo_flow_schedule_t *)ctx;
  if (!schedule || !flow || !stage || !stage->is_source) return TURBO_EINVAL;
  if (atomic_load_explicit(&schedule->started, memory_order_acquire)) return TURBO_EALREADY;
  tstr_freep(&schedule->source_name);
  schedule->source_name = tstr_dup(stage->name);
  if (!schedule->source_name) return TURBO_ENOMEM;
  schedule->flow = flow;
  ...
  if (schedule->manual_clock) return TURBO_OK;
  schedule->timer = turbo_timer_create(NULL);
  if (!schedule->timer) {
    atomic_store_explicit(&schedule->started, 0, memory_order_release);
    return TURBO_ENOMEM;
  }
  turbo_timer_set_data(schedule->timer, schedule);
  if (turbo_thread_create(&schedule->bootstrap_thread, flow_schedule_bootstrap_thread, schedule) !=
      TURBO_OK) {
    atomic_store_explicit(&schedule->started, 0, memory_order_release);
    turbo_timer_destroy(schedule->timer);
    schedule->timer = NULL;
    return TURBO_EIO;
  }
...
static void flow_schedule_stop(void *ctx, turbo_flow_t *flow,
  ...
  if (schedule->bootstrap_thread_started) {
    (void)turbo_thread_join(&schedule->bootstrap_thread);
    schedule->bootstrap_thread_started = 0;
  }
  turbo_mutex_lock(&schedule->mutex);
  turbo_timer_t *timer = schedule->timer;
  schedule->timer = NULL;
  turbo_mutex_unlock(&schedule->mutex);
...
static void flow_schedule_shutdown(void *ctx) {
  ...
  turbo_mutex_destroy(&schedule->mutex);
  tstr_freep(&schedule->source_name);
...
static int flow_schedule_validate(const turbo_flow_schedule_config_t *config,
  ...
  if (config->mode == TURBO_FLOW_SCHEDULE_CRON) {
    ...
    if (turbo_cron_parse_ex(config->cron_expression, cron_expr, error, sizeof(error)) !=
        TURBO_CRON_OK)
      return TURBO_EINVAL;
...
int turbo_flow_schedule_register_adapter(turbo_flow_t *flow, const char *name,
  ...
  turbo_mutex_init(&schedule->mutex);
  ...
  atomic_init(&schedule->last_status, TURBO_OK);
...
int turbo_flow_schedule_snapshot(const turbo_flow_schedule_t *schedule,
  ...
  if (!schedule || !out) return TURBO_EINVAL;
