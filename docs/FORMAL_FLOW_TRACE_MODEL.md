...
5. route evaluator 为每条输入 edge 产生且只产生一个布尔决策。
...
`RouteObservation` 包含一个 `StageResult`、一个 `RouteDecision` 和按 C runtime plan 顺序排列的
`List EdgeKind`。输出 mask 与输入 edges 等长，第 `i` 个布尔值等于对第 `i` 条 edge 调用
`edgeActive` 的结果。
...
9. `routeMask_length`：route mask 长度等于输入 edge 数量。

另外保留四个可计算 example，分别覆盖完整 Flow lifecycle、完整 task lifecycle、两路 fan-in
ready，以及 terminal 后第三次 resolution 被拒绝。example 由 kernel 归约检查，不使用外部测试框架。
